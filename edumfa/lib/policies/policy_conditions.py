...
from edumfa.lib.utils.compare import Comparators, compare_values
...
class ConditionHandleMissingData(Enum):
        ...
        * ``"description"``, a short description of the behaviour
        ...
        selection_dict = {
            cls.RAISE_ERROR.value: {
                ...
                "description": _(
                    "Raise an error if the data that is required to check the condition is not available."
                ),
            },
            cls.IS_TRUE.value: {
    ...
    def get_from_value(cls, value: str) -> "ConditionHandleMissingData":
        ...
        if value not in valid_handle_missing_data:
            log.error(
                f"Unknown handle missing data '{value}'. Valid values are: {valid_handle_missing_data}"
            )
...
class PolicyConditionClass:
    ...
    def __init__(
        ...
        value: str,
        active: bool,
        handle_missing_data: str = None,
        pass_if_inactive: bool = False,
    ):
    ...
    def section(self, section: str):
        if (
            section in ConditionSection.get_all_sections()
            or self._allow_invalid_parameters
        ):
            self._section = section
        else:
            log.error(
                f"Unknown section '{section}' set in condition. Valid sections are: "
    ...
    def key(self, key: str):
        if (isinstance(key, str) and len(key) > 0) or self._allow_invalid_parameters:
            ...
            raise ParameterError(
                f"Key must be a non-empty string. Got '{key}' of type '{type(key)}' instead."
            )
    ...
    def comparator(self, comparator: str):
        if (
            comparator in Comparators.get_all_comparators()
            or self._allow_invalid_parameters
        ):
            self._comparator = comparator
        else:
            log.error(
                f"Unknown comparator '{comparator}' set in condition. Valid comparators are: "
    ...
    def value(self, value: str):
        if (
            ...
            raise ParameterError(
                f"Value must be a non-empty string. Got '{value}' of type '{type(value)}' instead."
            )
    ...
    def active(self, active: bool):
        if isinstance(active, bool):
            ...
            raise ParameterError(
                f"Active must be a boolean. Got '{active}' of type '{type(active)}' instead."
            )
    ...
    def handle_missing_data(self, handle_missing_data: str):
        if handle_missing_data is None:
            ...
            try:
                self._handle_missing_data = ConditionHandleMissingData.get_from_value(
                    handle_missing_data
                )
            except ParameterError as e:
    ...
    def _do_handle_missing_data(
        self,
        policy_name: str,
        missing: str,
        object_name: str,
        available_keys: list = None,
    ) -> bool:
        """
        ...
        if missing == object_name:
            ...
            log_message = (
                f"Policy '{policy_name}' has a condition on the section '{self.section}' with key "
            ...
            log_message = (
                f"Unknown {self.section} key '{self.key}' referenced in condition of policy "
                f"'{policy_name}'."
            )
            if available_keys:
        ...
        if self.handle_missing_data is ConditionHandleMissingData.RAISE_ERROR:
            ...
            log.debug(
                log_message
                + " Evaluating condition as True, according to the policy definition."
            )
            if log_available_keys:
                log.debug(log_available_keys)
            return True
            ...
            log.debug(
                log_message
                + " Evaluating condition as False, according to the policy definition."
            )
            if log_available_keys:
                log.debug(log_available_keys)
            return False
            ...
            log.error(
                f"Unknown handle missing data {self.handle_missing_data} defined in condition of policy "
                f"{policy_name} for {object_name} and key '{self.key}'. Allowed values are: "
                f"{[ConditionHandleMissingData.get_valid_values()]}"
            )
            raise PolicyError(
                f"Unknown handle missing data {self.handle_missing_data} defined in condition of "
    ...
    def get_request_header_data(
        self, request_header: Union[EnvironHeaders, None]
    ) -> ConditionSectionData:
        """
    ...
    def match(
        ...
        request_header: Union[EnvironHeaders, None],
    ) -> bool:
        """
        ...
        if self.active:
            ...
            if self.section == ConditionSection.USERINFO:
                ...
                log.warning(
                    f"Policy '{policy_name}' has condition with unknown section: '{self.section}'"
                )
                raise PolicyError(
                    f"Policy '{policy_name}' has condition with unknown section"
                )
            ...
            if section_data.object_available:
                if section_data.value is not None:
                    try:
                        condition_matches = compare_values(
                            section_data.value, self.comparator, self.value
                        )
                    except Exception as e:
                        log.warning(
                            f"Error during handling the condition on {self.section} '{self.key}' "
                            f"of policy '{policy_name}': {e}"
                        )
                        raise PolicyError(
                            f"Invalid comparison in the {self.section} conditions of policy "
                            f"'{policy_name}'"
                        )
                else:
                    condition_matches = self._do_handle_missing_data(
                        policy_name,
                        missing=self.key,
                ...
                condition_matches = self._do_handle_missing_data(
                    policy_name,
                    missing=section_data.object_name,
                    object_name=section_data.object_name,
                )
