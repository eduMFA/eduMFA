...
def create_motp_url(key, user=None, realm=None, serial=""):
    ...
    return f"motp://edumfa:{url_label}?secret={otpkey}"
...
def create_google_authenticator_url(
    ...
    base_len = len(f"otpauth://{tokentype}/?secret={otpkey}&counter=1")
    allowed_label_len = MAX_QRCODE_LEN - base_len
    log.debug(f"we have got {allowed_label_len} characters left for the token label")
    # Deprecated
    ...
    return (
        f"otpauth://{tokentype}/{url_label}?secret={otpkey}&"
