...
#
# 2014 - 2016 Cornelius Kölbel <cornelius@privacyidea.org>
#
...
The SQL Audit Module is configured like this:
    ...
    EDUMFA_AUDIT_KEY_PRIVATE = "tests/testdata/private.pem"
    EDUMFA_AUDIT_KEY_PUBLIC = "tests/testdata/public.pem"
    EDUMFA_AUDIT_SERVERNAME = "your choice"

    Optional:
    EDUMFA_AUDIT_SQL_URI = "sqlite://"
    EDUMFA_AUDIT_SQL_TRUNCATE = True | False
    EDUMFA_AUDIT_SQL_COLUMN_LENGTH = {"user": 60, "info": 10 ...}

If the EDUMFA_AUDIT_SQL_URI is omitted the Audit data is written to the
token database.
"""
...
from sqlalchemy import MetaData, and_, asc, create_engine, desc, or_
from sqlalchemy.ext.compiler import compiles
from sqlalchemy.orm import scoped_session, sessionmaker
from sqlalchemy.sql.expression import FunctionElement

from edumfa.lib.auditmodules.base import Audit as AuditBase
from edumfa.lib.auditmodules.base import Paginate
...
class Audit(AuditBase):
    ...
    * ``EDUMFA_AUDIT_SQL_URI`` and
    * ``EDUMFA_AUDIT_SQL_OPTIONS``

    With ``EDUMFA_AUDIT_SQL_OPTIONS = {}`` You can pass options to the DB engine
    creation. If ``EDUMFA_AUDIT_SQL_OPTIONS`` is not set,
    ...
    * ``EDUMFA_CHECK_OLD_SIGNATURES``
    ...
    def _create_engine(self):
        """
        :return: a new SQLAlchemy engine connecting to the database specified in EDUMFA_AUDIT_SQL_URI.
        """
    ...
    def _get_logentry_attribute(key):
        ...
        sortname = {
            ...
            "info": LogEntry.info,
            "edumfa_server": LogEntry.edumfa_server,
    ...
    def audit_entry_to_dict(self, audit_entry):
        ...
        audit_dict["edumfa_server"] = audit_entry.edumfa_server
