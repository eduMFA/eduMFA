...
class Audit(AuditBase):
    ...
    def finalize_log(self):
        ...
        self.audit_data["timestamp"] = datetime.now(timezone.utc).replace(tzinfo=None).isoformat()
        if self.audit_data.get("startdate"):
            duration = datetime.now(timezone.utc).replace(tzinfo=None) - self.audit_data.get("startdate")
