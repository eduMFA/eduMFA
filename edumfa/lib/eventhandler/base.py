...
class BaseEventHandler:
        ...
        * successful or failed value.success
        ...
        cond = {
            ...
            CONDITION.USER_TOKEN_NUMBER: {
                "type": "str",
                "desc": _(
                    "This condition checks whether or not the number of "
        ...
        if hasattr(request, "User"):
            ...
            if user.is_empty() and serial:
                ...
                try:
                    ...
                    log.info(
                        f"Could not determine tokenowner for {serial}. Maybe the "
        ...
        if token_obj:
            ...
            if CONDITION.TOKEN_HAS_OWNER in conditions:
                ...
                if uid and check in ["True", True]:
                    ...
                    log.debug(
                        f"Condition token_has_owner for token {token_obj!r} "
                        "not fulfilled."
                    )
                    return False
            ...
            if CONDITION.TOKEN_IS_ORPHANED in conditions:
                ...
                if orphaned and check in ["True", True]:
                    ...
                    log.debug(
                        f"Condition token_is_orphaned for token {token_obj!r} not "
                        "fulfilled."
                    )
                    return False
    ...
    def do(self, action, options=None):
        ...
        :return:
        """
        log.info(f"In fact we are doing nothing, be we presume we are doing{action}")
        return True
