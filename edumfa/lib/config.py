...
#
# 2014 - 2016 Cornelius Kölbel <cornelius.koelbel@netknights.it>
#
...
def get_resolver_classes():
    ...
    [<class 'edumfa.lib.resolvers.PasswdIdResolver.IdResolver'>,
    <class 'edumfa.lib.resolvers.UserIdResolver.UserIdResolver'>]
...
def get_token_class_dict():
    ...
    ({'edumfa.lib.tokens.hotptoken.HotpTokenClass':
      <class 'edumfa.lib.tokens.hotptoken.HotpTokenClass'>,
      'edumfa.lib.tokens.totptoken.TotpTokenClass':
      <class 'edumfa.lib.tokens.totptoken.TotpTokenClass'>},
    ...
    for module in modules:
        for name in dir(module):
            ...
            if (
                ...
                try:
                    class_name = f"{module.__name__}.{obj.__name__}"
...
def get_machine_resolver_class_dict():
    ...
    ({'edumfa.lib.machines.hosts.HostsMachineResolver':
      <class 'edumfa.lib.machines.hosts.HostsMachineResolver'>},
     {'edumfa.lib.machines.hosts.HostsMachineResolver':
      'hosts'})
...
def get_caconnector_class_dict():
    ...
    ({'edumfa.lib.caconnectors.localca.LocalCAConnector':
      <class 'edumfa.lib.caconnectors.localca.LocalCAConnector'>},
     {'edumfa.lib.caconnectors.localca.LocalCAConnector':
      'local'})
...
def get_resolver_class_dict():
    ...
    ({'edumfa.lib.resolvers.PasswdIdResolver.IdResolver':
      <class 'edumfa.lib.resolvers.PasswdIdResolver.IdResolver'>,
      'edumfa.lib.resolvers.PasswdIdResolver.UserIdResolver':
      <class 'edumfa.lib.resolvers.UserIdResolver.UserIdResolver'>},
...
def get_resolver_list():
    ...
    module_list.add("edumfa.lib.resolvers.PasswdIdResolver")
    module_list.add("edumfa.lib.resolvers.LDAPIdResolver")
    module_list.add("edumfa.lib.resolvers.SCIMIdResolver")
    module_list.add("edumfa.lib.resolvers.SQLIdResolver")
    module_list.add("edumfa.lib.resolvers.HTTPResolver")

    # Dynamic Resolver modules
    # TODO: Migration
    # config_modules = config.get("eduMFAResolverModules", '')
    config_modules = None
    log.debug(f"{config_modules}")
    if config_modules:
...
def get_machine_resolver_class_list():
    ...
    class_list.append("edumfa.lib.machines.hosts.HostsMachineResolver")
    class_list.append("edumfa.lib.machines.ldap.LdapMachineResolver")
    return class_list
...
def get_token_list():
    ...
    module_list.add("edumfa.lib.tokens.daplugtoken")
    module_list.add("edumfa.lib.tokens.hotptoken")
    module_list.add("edumfa.lib.tokens.motptoken")
    module_list.add("edumfa.lib.tokens.passwordtoken")
    module_list.add("edumfa.lib.tokens.applicationspecificpasswordtoken")
    module_list.add("edumfa.lib.tokens.remotetoken")
    module_list.add("edumfa.lib.tokens.daypasswordtoken")
    module_list.add("edumfa.lib.tokens.spasstoken")
    module_list.add("edumfa.lib.tokens.sshkeytoken")
    module_list.add("edumfa.lib.tokens.totptoken")
    module_list.add("edumfa.lib.tokens.yubicotoken")
    module_list.add("edumfa.lib.tokens.yubikeytoken")
    module_list.add("edumfa.lib.tokens.radiustoken")
    module_list.add("edumfa.lib.tokens.smstoken")
    module_list.add("edumfa.lib.tokens.emailtoken")
    module_list.add("edumfa.lib.tokens.registrationtoken")
    module_list.add("edumfa.lib.tokens.certificatetoken")
    ...
    if dynamic_token_modules:
        # In the edumfa.cfg you can specify a list or set of 3rd party token modules like
        #    EDUMFA_TOKEN_MODULES = [ "myproj.tokens.tok1", "myproj.tokens.tok2" ]
...
def get_token_module_list():
    ...
    log.debug(f"using the module list: {module_list}")
    ...
    for mod_name in module_list:
        ...
        # else:
        try:
            log.debug(f"import module: {mod_name}")
...
def get_machine_resolver_module_list():
    ...
    log.debug(f"using the class list: {class_list}")
...
def set_edumfa_config(key, value, typ="", desc=""):
    """
...
def set_prepend_pin(prepend=True):
    ...
    :return: None
    """
    set_edumfa_config("PrependPin", prepend)
...
def get_edumfa_nodes():
    """
    This returns the list of the nodes, including the own local node name
    :return: list of nodes
    """
    own_node_name = get_edumfa_node()
    nodes = get_app_config_value("EDUMFA_NODES", [])[:]
...
def import_config(data, name=None):
    ...
    log.debug(f"Import server config: {data}")
    res = {}
