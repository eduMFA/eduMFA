...
#
"""
This is the library with base functions for eduMFA.
...
def sanity_name_check(name, name_exp=r"^[A-Za-z0-9_\-\.]+$"):
    ...
    if re.match(name_exp, name) is None:
        raise Exception("non conformant characters in the name: %r (not in %s)" % (name, name_exp))
    return True
        ...
        if k not in exclude_params:
            if k.startswith('type.') is True:
                ...
                if k in config_description:
                    ...
                    log.warning("the passed key '{0!s}' is not a parameter for the {1!s} type '{2!s}'".format(k, module, type))
...
def check_proxy(path_to_client, proxy_settings):
    ...
    :param path_to_client: A list of IPAddress objects containing all proxy hops, starting with the current HTTP
                           ...
                           192.168.1.3 before finally reaching eduMFA.
    ...
    try:
        ...
        log.error("Error parsing the OverrideAuthorizationClient setting: {0!s}! The IP addresses need to be comma separated. Fix this. The client IP will not be mapped!".format(proxy_settings))
        log.debug(f"{traceback.format_exc()!s}")
...
def get_client_ip(request, proxy_settings):
    ...
    # If a client C talks to a proxy P1, which in turn talks to a proxy P2, which talks to eduMFA,
    ...
    # To prevent IP spoofing, eduMFA allows to configure a list of proxies that are allowed to override the
...
def get_module_class(package_name, class_name, check_method=None):
    ...
    example::
        ...
        get_module_class("edumfa.lib.auditmodules.sqlaudit", "Audit", "log")

        get_module_class("edumfa.lib.monitoringmodules.sqlstats", "Monitoring")
    ...
    if check_method and not hasattr(klass, check_method):
        raise NameError("Class AttributeError: {0}.{1} instance has no attribute '{2}'".format(package_name, class_name, check_method))
    return klass


def get_version_number():
    """
    returns the edumfa version
    """
    version = "unknown"
    try:
        version = importlib_metadata.version("edumfa")
    except:
        log.info("We are not able to determine the edumfa version number.")
    return version
