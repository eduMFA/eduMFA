...
from edumfa.lib.framework import (get_app_local_store, get_app_config_value,
                                       get_app_config)
from edumfa.lib.utils import (to_unicode, to_bytes, hexlify_and_unicode,
...
def pass_hash(password):
    ...
    """
    DEFAULT_HASH_ALGO_PARAMS.update(get_app_config_value("EDUMFA_HASH_ALGO_PARAMS",
                                                         default={}))
    pass_ctx = CryptContext(get_app_config_value("EDUMFA_HASH_ALGO_LIST",
...
def hash_with_pepper(password):
    ...
    :rtype: str
    """
    key = get_app_config_value("EDUMFA_PEPPER", "missing")
...
def init_hsm():
    ...
    The config file edumfa.cfg may contain EDUMFA_HSM_MODULE and parameters like:
    EDUMFA_HSM_MODULE_MODULE
    EDUMFA_HSM_MODULE_SLOT_ID...
...
def create_hsm_object(config):
    ...
    The config dictionary are the values that appear in edumfa.cfg.

    It is needed EDUMFA_HSM_MODULE and all other values
    EDUMFA_HSM_MODULE_* depending on the module implementation.
    ...
    hsm_module_name = config.get("EDUMFA_HSM_MODULE",
    ...
    if class_name == "DefaultSecurityModule":
        ...
        # get all parameters by splitting every config entry starting with EDUMFA_HSM_MODULE_
        # and pass this as a config object to hsm_class.
        hsm_parameters = {}
        for key in config.keys():
            if key.startswith("EDUMFA_HSM_MODULE_"):
                param = key[len("EDUMFA_HSM_MODULE_"):].lower()
