...
def set_periodic_task(
    ...
    :type active: bool
    :param retry_if_failed: true if edumfa should retry to execute this periodic task if it fails
                            false if edumfa should just try onetime regardless the failing of the task
    ...
    try:
        ...
        raise ParameterError(f"Invalid interval: {e}")
    if ordering < 0:
        raise ParameterError(f"Invalid ordering: {ordering}")
    if id is not None:
...
def get_scheduled_periodic_tasks(node, current_timestamp=None, interval_tzinfo=None):
    ...
    scheduled_ptasks = []
    log.debug(f"Collecting periodic tasks to run at {current_timestamp.isoformat()}")
...
def import_periodictask(data, name=None):
    ...
    log.debug(f"Import periodictask config: {data}")
    for res_data in data:
        ...
        log.info(f'Import of periodictask "{res_data["name"]}" finished, id: {rid}')
