...
#
# 2017 - 2020 Cornelius Kölbel <cornelius.koelbel@netknights.it>
#
...
def add_to_cache(username, realm, resolver, password):
    ...
    first_auth = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
...
def update_cache(cache_id):
    last_auth = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
...
def cleanup(minutes):
    ...
    :return:
    """
    cleanuptime = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None) - datetime.timedelta(minutes=minutes)
