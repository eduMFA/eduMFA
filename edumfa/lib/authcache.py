...
def update_cache(cache_id):
    ...
    db.session.query(AuthCache).filter(AuthCache.id == cache_id).update({"last_auth": last_auth, AuthCache.auth_count: AuthCache.auth_count + 1})
    db.session.commit()
...
def verify_in_cache(username, realm, resolver, password, first_auth=None, last_auth=None, max_auths=0):
    """
