...
class HttpSMSProvider(ISMSProvider):
    def submit_message(self, phone, message):
        ...
        log.debug(
            "issuing request with parameters {params} (data: {data}, json: {json_param}), headers {headers}, method {method} andauthentication {basic_auth} to url {url}.".format(
                **log_dict
            )
        )
    ...
    def parameters(cls):
        ...
        params = {
            ...
            "parameters": {
                ...
                "PROXY": {
                    "description": _(
                        "An optional proxy string. DEPRECATED. Do not use this anymore. Rather use HTTP_PROXY for http connections and HTTPS_PROXY for https connection. The PROXY"
