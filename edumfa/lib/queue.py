...
def register_app(app):
    """
    Register the app ``app`` with the global job collector, if a EDUMFA_JOB_QUEUE_CLASS is non-empty.
...
def get_job_queue():
    ...
    if "job_queue" in store:
        ...
        raise ServerError("eduMFA has no job queue configured!")
