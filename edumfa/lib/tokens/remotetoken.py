...
forwards an authentication request to another eduMFA server.
...
class RemoteTokenClass(TokenClass):
    """
    The Remote token forwards an authentication request to another eduMFA
    ...
    eduMFA server or on the remote server.
    ...
    def update(self, param):
        ...
        if remote_server_id is not None:
            if get_edumfaserver(id=remote_server_id):
    ...
    def check_otp(self, otpval, counter=None, window=None, options=None):
        ...
        if remote_server_id:
            pi_server_obj = get_edumfaserver(id=int(remote_server_id))
        else:
            # Deprecated
        ...
        log.debug("checking OTP len:%r remotely on server: %r, serial: %r, user: %r" %
