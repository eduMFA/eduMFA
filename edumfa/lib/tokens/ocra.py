...
class OCRASuite(object):
    def __init__(self, ocrasuite):
        ...
        if len(algo_crypto_data) != 3:
            raise Exception(
                "The OCRAsuite consists of three fields 'algorithm', 'cryptofunction' and 'datainput' delimited by ':'"
            )
        ...
        if len(hotp_sha_trunc) != 3:
            raise Exception(
                "The cryptofunction consists of three fields 'HOTP', 'SHA' and 'Truncation' delimited by '-'"
            )
        ...
        if self.truncation not in [0, 4, 5, 6, 7, 8, 9, 10]:
            raise Exception(
                f"Only truncation of 0 or 4-10 is allowed. You specified {self.truncation}"
            )
        ...
        if len(counter_input_signature) not in [1, 2, 3]:
            raise Exception(
                "Error in datainput. The datainput must consist of 1, 2 or 3 fields separated by '-'"
            )
        ...
        if self.challenge_type not in ["QA", "QH", "QN"]:
            raise Exception(
                "Error in challenge. The challenge must start with QA, QN or QH. You specified %s"
                % self.challenge
            )
        ...
        try:
            ...
            raise Exception(
                f"The last characters in the challenge must be a number. You specified {self.challenge}"
            )

        if self.challenge_length < 4 or self.challenge_length > 64:
            raise Exception(
                "The length of the challenge must be specified between 4 and 64. You specified %s"
        ...
        if not self.signature:
            ...
            if self.signature_type not in ["P", "S", "T"]:
                raise Exception(
                    f"The signature needs to be P, S or T. You specified {self.signature_type}"
                )
            if self.signature_type == "P":
                ...
                try:
                    ...
                    raise Exception(
                        "You must specify a valid number in the timestamp in the signature."
                    )
