...
class WebAuthnTokenClass(TokenClass):
            ...
            "policy": {
                SCOPE.AUTH: {
                    ...
                    WEBAUTHNACTION.USERNAMELESS_AUTHN: {
                        "type": "bool",
                        "desc": _(
                            "Whether usernameless authentication via the Passkeys Autofill UI, also known as "
                    ...
                    WEBAUTHNACTION.USERNAMELESS_REALM_POLICY: {
                        "type": "bool",
                        "desc": _(
                            "Whether to enforce the WebAuthn authentication policies for the relying party ID "
                ...
                SCOPE.ENROLL: {
                    ...
                    WEBAUTHNACTION.AUTHENTICATOR_RESIDENT_KEY: {
                        "type": "str",
                        "desc": _(
                            "Whether to request a resident key. Note: Passkeys are always resident keys."
                        ),
                        "group": WEBAUTHNGROUP.WEBAUTHN,
    ...
    def _get_webauthn_user(self, user):
        return WebAuthnUser(
            ...
            icon_url=(
                IMAGES.get(self.token.description.lower().split()[0], "")
        ...
        if not (reg_data and client_data):
            ...
            if registration_client_extensions:
                try:
                    ...
                    resident_key = "rk" in extensions and (
                        extensions["rk"] or extensions["rk"] == "true"
                    )
                    if resident_key:
    ...
    def get_init_detail(self, params=None, user=None):
        ...
        if self.token.rollout_state == ROLLOUTSTATE.CLIENTWAIT:
            ...
            if not user:
                raise ParameterError(
                    "Failed to create a WebAuthn token."
            ...
            tokenlabel = params.get(
                WEBAUTHNACTION.TOKENLABEL, "{givenname} {surname} ({user})"
            )
            label = (
                tokenlabel.replace("<s>", self.token.serial)
                .replace("<u>", self.user.login)
                .replace("<r>", self.user.realm)
            )
            label = label.format(
                serial=self.token.serial,
    ...
    def check_userless_otp(options=None):
        if is_webauthn_assertion_response(options):
            ...
            try:
                ...
                if count != -1:
                    token.add_tokeninfo(
                        ACTION.LASTAUTH, datetime.datetime.now(tzlocal())
                    )
