...
class SmsTokenClass(HotpTokenClass):
            ...
            "policy": {
                SCOPE.AUTH: {
                    ...
                    SMSACTION.SMSPOSTCHECKTEXT: {
                        "type": "str",
                        "desc": _(
                            "The message that will be submitted for "
                ...
                SCOPE.USER: {
                    SMSACTION.GATEWAYS: {
                        "type": "str",
                        "desc": _("Choose the gateways the user is allowed to set.")
                        + f" ({' '.join(sms_gateways)})",
                    }
                },
                SCOPE.ENROLL: {
    ...
    def check_otp(self, anOtpVal, counter=None, window=None, options=None):
        ...
        if ret >= 0:
            if self._get_auto_sms(options):
                ...
                log.debug(f"AutoSMS: send new SMS: {success}")
    ...
    def _send_sms(self, message="<otp>", options=None):
        ...
        if sms_gateway_identifier:
            ...
            log.debug(f"smsprovider: {SMSProvider}, class: {SMSProviderClass}")
            ...
            try:
                ...
                log.debug(f"loading SMS configuration for class {sms}")
        ...
        if sms.send_to_uid():
            ...
            if not uid:
                log.warning(f"Token {self.token.serial} does not have a uid!")
            destination = uid
        else:
            ...
            if not phone:  # pragma: no cover
                log.warning(f"Token {self.token.serial} does not have a phone number!")
            destination = phone
