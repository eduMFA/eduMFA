    ...
    **Step 2**:
    ...
    def _api_endpoint_post(cls, request_data):
        ...
        if all(k in request_data for k in ("fbtoken", "pubkey")):
            ...
            try:
                ...
                log.debug(f"{traceback.format_exc()!s}")
                log.info(
                    'The following error occurred during the signature check: "{0!r}"'.format(
                        e
                    )
                )
        ...
        try:
            ...
            if allow_polling == PushAllowPolling.TOKEN:
                if not is_true(tok.get_tokeninfo(POLLING_ALLOWED, default="True")):
                    log.debug(
                        "Polling not allowed for pushtoken {0!s} due to tokeninfo.".format(
                            serial
                        )
                    )
                    raise PolicyError("Polling not allowed!")
            ...
            if not registration_url:
                raise ResourceNotFoundError(
                    "There is no registration_url defined for the  pushtoken {0!s}. You need to define a push_registration_url in an enrollment policy.".format(
                        serial
                    )
                )
            options = {"g": g}
    ...
    def create_challenge(self, transactionid=None, options=None):
        ...
        if fb_identifier:
            ...
            log.warning(
                "The token {0!s} has no tokeninfo {1!s}. The message could not be sent.".format(
