...
ERROR_CHALLENGE_TEXT = _(
    "Use the polling feature of your unsupported privacyIDEA Authenticator App to check for a new Login request.")
...
def create_push_token_url(
    ...
    return "otpauth://blafasel/{label!s}?url={url!s}&ttl={ttl!s}&issuer={issuer!s}{extra}".format(
        label=url_label,
        issuer=url_issuer,
        url=url_url,
        ttl=ttl,
...
class PushTokenBaseClass(TokenClass):
    """
    ...
    **Step 2**:
    ...
    def _build_smartphone_data(cls, serial, challenge, registration_url, pem_privkey, options):
        ...
        :rtype: dict
        """
        sslverify = get_action_values_from_options(SCOPE.AUTH, cls.KEY_CLASS.SSL_VERIFY, options) or "1"
        sslverify = getParam({"sslverify": sslverify}, "sslverify", allowed_values=["0", "1"], default="1")
        message_on_mobile = get_action_values_from_options(SCOPE.AUTH, cls.KEY_CLASS.MOBILE_TEXT,
                                                           options) or DEFAULT_MOBILE_TEXT
        title = get_action_values_from_options(SCOPE.AUTH, cls.KEY_CLASS.MOBILE_TITLE, options) or "eduMFA"
        smartphone_data = {
            "nonce": challenge,
    ...
    def get_class_info(cls, key=None, ret="all"):
        ...
        res = {
            ...
            "policy": {
                SCOPE.ENROLL: {
                    cls.KEY_CLASS.FIREBASE_CONFIG: {
                        "type": "str",
                        "desc": _("The configuration of your Firebase application."),
                        "group": "PUSH",
                        "value": [POLL_ONLY] + [gw.identifier for gw in gws],
                    },
                    cls.KEY_CLASS.REGISTRATION_URL: {
                        "required": True,
                        "type": "str",
                        "group": "PUSH",
                        "desc": _(
                            "The URL the Push App should contact in the second enrollment step. Usually it is the endpoint /ttype/push of the eduMFA server."),
                    },
                    cls.KEY_CLASS.TTL: {
                        "type": "int",
                        "group": "PUSH",
                    ...
                    f"{cls.CLASS_TYPE}_{ACTION.FORCE_APP_PIN}": {
                        "type": "bool",
                        "group": "PUSH",
                ...
                SCOPE.AUTH: {
                    ...
                    cls.KEY_CLASS.WAIT: {
                        "type": "int",
                        "desc": _(
                            "Wait for number of seconds for the user to confirm the challenge in the first request."),
                        "group": "PUSH",
                    },
                    cls.KEY_CLASS.ALLOW_POLLING: {
                        "type": "str",
    ...
    def update(self, param, reset_failcount=True):
        ...
        if "serial" in upd_param and "fbtoken" in upd_param and "pubkey" in upd_param:
            ...
            self.add_tokeninfo(self.KEY_CLASS.FIREBASE_CONFIG, param.get(self.KEY_CLASS.FIREBASE_CONFIG))
        else:
    ...
    def get_init_detail(self, params=None, user=None):
        ...
        sslverify = getParam(params, self.KEY_CLASS.SSL_VERIFY, allowed_values=["0", "1"], default="1")
        ...
        if self.token.rollout_state == ROLLOUTSTATE.CLIENTWAIT:
            ...
            registration_url = getParam(params, self.KEY_CLASS.REGISTRATION_URL, optional=False)
            ttl = getParam(params, self.KEY_CLASS.TTL, default="10")
            # Get the values from the configured PUSH config
            fb_identifier = params.get(self.KEY_CLASS.FIREBASE_CONFIG)
    ...
    def _api_endpoint_get(cls, g, request_data):
        ...
        allow_polling = get_action_values_from_options(SCOPE.AUTH, cls.KEY_CLASS.ALLOW_POLLING,
        ...
        try:
            ...
            registration_url = get_action_values_from_options(SCOPE.ENROLL, cls.KEY_CLASS.REGISTRATION_URL,
                                                              options={"g": g})
    ...
    def is_challenge_request(self, passw, user=None, options=None):
        ...
        """
        if options.get(self.KEY_CLASS.WAIT):
    ...
    def create_challenge(self, transactionid=None, options=None):
        ...
        res = True
        fb_identifier = self.get_tokeninfo(self.KEY_CLASS.FIREBASE_CONFIG)
        if fb_identifier:
            ...
            if options.get("session") != CHALLENGE_SESSION.ENROLLMENT:
                if fb_identifier != POLL_ONLY:
                    ...
                    registration_url = get_action_values_from_options(SCOPE.ENROLL, self.KEY_CLASS.REGISTRATION_URL,
                                                                      options=options)
            ...
            allow_polling = get_action_values_from_options(SCOPE.AUTH, self.KEY_CLASS.ALLOW_POLLING,
                                                           options=options) or PushAllowPolling.ALLOW
            if (allow_polling == PushAllowPolling.ALLOW or (allow_polling == PushAllowPolling.TOKEN and is_true(
                    self.get_tokeninfo(POLLING_ALLOWED, default="True")))) or res:
            ...
            log.warning(
                f"The token {self.token.serial!s} has no tokeninfo {self.KEY_CLASS.FIREBASE_CONFIG!s}. The message could not be sent.")
            message += f" {ERROR_CHALLENGE_TEXT}"
            if is_true(options.get("exception")):
    ...
    def authenticate(self, passw, user=None, options=None):
        ...
        if pin_match:
            if not options.get("valid_token_num"):
                ...
                waiting = int(options.get(self.KEY_CLASS.WAIT, 20))
    ...
    def enroll_via_validate(cls, g, content, user_obj):
        ...
        # We also store the Firebase config, that was used during the enrollment.
        token_obj.add_tokeninfo(cls.KEY_CLASS.FIREBASE_CONFIG, params.get(cls.KEY_CLASS.FIREBASE_CONFIG))
...
class PushToken(PushTokenBaseClass):
    CLASS_TYPE = "push"
    CLASS_PREFIX = "PIPU"
