...
@log_with(log)
def create_push_token_url(url=None, ttl=10, issuer="eduMFA", serial="mylabel",
...
class PushTokenClass(TokenClass):
    ...
    **Step 2**:
    ...
    def _build_smartphone_data(cls, serial, challenge, registration_url, pem_privkey, options):
        ...
        # This allows notification processing on iOS without polling the server
        if "content-available" in options:
          smartphone_data["content-available"] = options["content-available"] 

        # This allows sending a custom message to the application informing where the user is logging in from
    ...
    def api_endpoint(cls, request, g):
        ...
        if request.method == 'POST':
            ...
            raise eduMFAError('Method {0!s} not allowed in \'api_endpoint\' '
