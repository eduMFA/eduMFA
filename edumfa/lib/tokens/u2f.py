...
def parse_registration_data(reg_data, verify_cert=True):
    ...
    if verify_cert:
        if (
            ...
            log.error(f"The certificate is not valid. {not_before} -> {not_after}")
    ...
    log.debug(f"This attestation certificate registered: {cdump}")
...
def check_registration_data(
    ...
    try:
        ...
        if isinstance(public_key_cryptography, ec.EllipticCurvePublicKey):
            ...
            )  # Ensure this matches the hash used for signing
...
def check_response(
    ...
    try:
        ...
        log.error(f"Bad signature for app_id {app_id}")
        res = False
    return res
