...
# The directory for logging can be configured via EDUMFA_HSM_MODULE_LOCK_DIR in edumfa.cfg
...
def hsm_lock(timeout=DEFAULT_TIMEOUT, lock_dir=DEFAULT_LOCK_DIR):
    ...
    while timeout > 0:
        ...
        try:
            ...
            log.info(
                f"Can not get the lock on {lock_dir}. Can not initialize the HSM, yet."
            )
            time.sleep(1)
        finally:
            # Cleanup
...
class EncryptKeyHardwareSecurityModule(DefaultSecurityModule):  # pragma: no cover
    def __init__(self, config=None, logout=True):
        ...
        The values can be configured in the edumfa.cfg file like this:
        ...
        # EDUMFA_HSM_MODULE_SLOT = 1
        EDUMFA_HSM_MODULE_KEYID = 1
        # Alternative to KEYID -- only use one of both!
        EDUMFA_HSM_MODULE_KEYLABEL = "my secret key"
        EDUMFA_HSM_MODULE_PASSWORD = '123456'
        ...
        with hsm_lock(timeout=timeout, lock_dir=lock_dir):
                ...
                if len(slotlist) == 1:
                    ...
                    for slot in slotlist:
                        ...
                        if slotinfo.slotDescription.startswith(self.slotname):
                            self.slot = slot
                            break
            log.info(f"Using slot {self.slot}")
            ...
            log.info(f"Setting up slot {self.slot}: '{slotinfo.slotDescription}'")
            ...
            try:
                ...
                if str(e).startswith("CKR_USER_ALREADY_LOGGED_IN"):
                    ...
                    raise e
            log.info(f"Logged into slot {self.slot}")
    ...
    def _get_private_key(self):
        ...
        log.debug(f"Found {len(objs)} private keys.")
        return objs[0]
    ...
    def _encrypt_file(self, infile, outfile):
        ...
        log.debug(f"Found {len(objs)} public keys.")
        for obj in objs:
            log.debug("========================================================")
    ...
    def _listkeys(self, keytype="public"):
        ...
        log.debug(f"Found {len(objs)} keys.")
        for obj in objs:
            log.debug("========================================================")
            log.debug(f"Found object {obj}")
