...
try:
    ...
    from edumfa.lib.caconnectors.caservice_pb2 import (GetCAsRequest,
                                                            GetTemplatesRequest,
                                                            GetCSRStatusRequest,
                                                            GetCSRStatusReply,
                                                            SubmitCSRRequest,
...
class MSCAConnector(BaseCAConnector):
    ...
    def _connect_to_worker(self):
        ...
        if self.use_ssl:
            ...
            if not (self.ssl_ca_cert and self.ssl_client_cert and self.ssl_client_key):
                log.error("For a secure connection we need 'ssl_ca_cert', 'ssl_client_cert' and 'ssl_client_key'. The following configuration seems incomplete: ({0!s}, {1!s}, {2!s})".format(self.ssl_ca_cert, self.ssl_client_cert,
                                                         self.ssl_client_key))
                ...
                try:
                    ...
                    log.error(f'Could not decrypt TLS key with given password. ({e!s})')
                    raise CAError('Invalid TLS configuration for MSCA worker.')
                except TypeError as e:
                    log.error(f"Faulty configuration in CA '{self.name!s}'. Trying to use an encrypted private key without providing a password (or vice versa)! The CA connector will not work! ({e!s})")
                    raise CAError('Invalid TLS configuration for MSCA worker.')
    ...
    def create_ca(name):  # pragma: no cover
        ...
        * if a http_proxy is used
        ...
        while True:
            config.hostname = input("Hostname of the eduMFA MS CA worker: ")
            config.port = input("Port of the eduMFA MS CA worker: ")
