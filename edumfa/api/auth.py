...
from edumfa.lib.auth import (check_webui_user, ROLE, verify_db_admin,
                                  db_admin_exist)
...
from edumfa.api.lib.postpolicy import (postpolicy, get_webui_settings, add_user_detail_to_response, check_tokentype,
                                            check_tokeninfo, check_serial, no_detail_on_fail, no_detail_on_success,
                                            get_webui_settings)
from edumfa.api.lib.prepolicy import (is_remote_user_allowed, prepolicy,
...
def get_auth_token():
    ...
    **Response for failed authentication**:
    ...
    # "eduMFA" = The admin or the user is authenticated against eduMFA
    ...
    if (user == username) and is_remote_user_allowed(request) != REMOTE_USER.DISABLE:
        ...
        if password is None:
            ...
            if db_admin_exist(user_obj.login) and user_auth and realm == get_default_realm():
                ...
                log.warning(f"A user '{user_obj.login!s}' exists as local admin a"
                            "nd as user in your default realm!")
            if role == ROLE.ADMIN:
