...
def init():
    ...
    **2 Step Enrollment**
    ...
    the eduMFA server and from a second part generated by the smartphone
    app/client.
    ...
    two parts. See
    :func:`~edumfa.lib.tokenclass.TokenClass.generate_symmetric_key`
    ...
    * :func:`~edumfa.lib.tokenclass.TokenClass.verify_enrollment`
    * :func:`~edumfa.lib.tokenclass.TokenClass.prepare_verify_enrollment`
...
def list_api():
    ...
    g.audit_object.log({"info": f"realm: {allowed_realms}"})
...
def set_api(serial=None):
    ...
    if use_timeshift is not None:
        g.audit_object.add_to_log(
            {"action_detail": f"use_timeshift={use_timeshift!r}, "}
        )
...
def loadtokens_api(filename=None):
    ...
    if file_contents == "":
        log.error(f"Error loading/importing token file. file {filename} empty!")
...
def get_serial_by_otp_api(otp=None):
    ...
    g.audit_object.log(
        {
            "success": True,
            "info": f"get {serial} by OTP. {count} tokens",
        }
    )
...
def set_tokeninfo_api(serial, key):
    ...
    g.audit_object.log({"serial": serial})
    g.audit_object.log({"action_detail": f"key={key}, value={value}"})
...
def modify_tokeninfo_api(serial):
    ...
    if type(tokeninfo) is not dict:
        raise ParameterError(
            "Parameter 'info' must be a list of key, value pairs (dictionary). but is of type {0!r}",
            type(tokeninfo),
        )
    else:
        g.audit_object.log({"serial": serial})
        for key, value in tokeninfo.items():
            g.audit_object.add_to_log({"action_detail": f"'{key}':'{value}', "})
            if value is None:
                count += delete_tokeninfo(serial, key)
            else:
                count += add_tokeninfo(serial, key, value)
