...
def set_random_pin(request=None, action=None):
    ...
    if len(pin_pols) == 0:
        ...
        if len(pol_contents) == 1:
            ...
            log.debug(f"Creating random OTP PIN with length {list(pin_pols)[0]}")
...
def init_random_pin(request=None, action=None):
    ...
    if len(pin_pols) == 1:
        ...
        for handle_pol in handle_pols:
            log.debug(f"Handle the random PIN with the class {handle_pol}")
...
def sms_identifiers(request=None, action=None):
    ...
    if sms_identifier:
        ...
        if sms_identifier not in gateway_identifiers:
            log.warning(f"{sms_identifier} not in {gateway_identifiers}")
            raise PolicyError(
...
def init_token_defaults(request=None, action=None):
    ...
    log.debug(f"Adding default settings {default_settings} for token type {ttype}")
...
def init_tokenlabel(request=None, action=None):
    ...
    params : { "tokenissuer": "eduMFA instance" }
    ...
    if verify and serial:
        ...
        if len(tokenobj_list) == 1:
            ...
            if tokenobj.rollout_state == ROLLOUTSTATE.VERIFYPENDING:
                log.debug(f"Verifying the token enrollment for token {serial}.")
                r = tokenobj.verify_enrollment(verify)
                log.info(f"Result of enrollment verification for token {serial}: {r}")
                if r:
...
def check_max_token_user(request=None, action=None):
    ...
    if user_object.login:
        ...
        limit_list = Match.user(
            g,
            scope=SCOPE.ENROLL,
            action=f"{tokentype.lower()}_{ACTION.MAXTOKENUSER}",
            user_object=user_object,
        ...
        limit_list = Match.user(
            g,
            scope=SCOPE.ENROLL,
            action=f"{tokentype}_{ACTION.MAXACTIVETOKENUSER}",
            user_object=user_object,
        ).action_values(unique=False, write_to_audit_log=False)
        if limit_list:
...
def auditlog_age(request=None, action=None):
    ...
    for aa in audit_age:
        ...
        log.debug(f"auditlog_age: {timelimit_s}")
...
def mangle(request=None, action=None):
    ...
    for mangle_pol_action in mangle_pols:
        ...
        if mangle_value:
            log.debug(f"mangling authentication data: {mangle_key}")
...
def check_external(request=None, action="init"):
    ...
    module = None
    try:
        module_func = current_app.config.get("EDUMFA_INIT_CHECK_HOOK")
        if module_func:
            module_name = ".".join(module_func.split(".")[:-1])
            module = importlib.import_module(module_name)
            function_name = module_func.split(".")[-1]
    except Exception as exx:
        log.error(f"Error importing external check function: {exx}")
...
def is_remote_user_allowed(req, write_to_audit_log=True):
    ...
    if (
        not user
        and not req.headers.get("Remote-User") is None
        and not req.headers.get("Remote-User") == ""
    ):
...
def save_client_application_type(request, action):
    ...
    save_clientapplication(client_ip, f"{ua}" or "unknown")
    return True
...
def pushtoken_wait(request, action):
    ...
    waiting = Match.user(
        g,
        scope=SCOPE.AUTH,
        action=PushTokenClass.PUSH_ACTION.WAIT,
...
def indexedsecret_force_attribute(request, action):
    ...
    if ttype and ttype.lower() == "indexedsecret" and request.User:
        ...
        attributes = Match.admin_or_user(
            g,
            f"indexedsecret_{PIIXACTION.FORCE_ATTRIBUTE}",
            user_obj=request.User,
        ).action_values(unique=True)
        if not attributes:
...
def webauthntoken_request(request, action):
    ...
    if webauthn:
        ...
        usernameless_activated = Match.realm(
            g, scope=scope, action=WEBAUTHNACTION.USERNAMELESS_AUTHN, realm=realm
        ).any()
        ...
        if realm and scope == SCOPE.AUTH and is_usernameless:
            ...
            request.all_data[WEBAUTHNACTION.USERNAMELESS_REALM_POLICY] = (
                is_usernameless_realm_policy
            )
        ...
        if not is_usernameless_realm_policy:
            ...
            if WEBAUTHNACTION.TIMEOUT in actions:
                timeout_policies = Match.realm(
                    g, scope=scope, action=WEBAUTHNACTION.TIMEOUT, realm=realm
                ).action_values(unique=True)
                timeout = (
                    int(list(timeout_policies)[0])
                    if timeout_policies
...
def webauthntoken_auth(request, action):
    ...
    if (
        ...
        request.all_data[
            f"{WebAuthnTokenClass.get_class_type()}_{ACTION.CHALLENGETEXT}"
        ] = challengetext
...
def webauthntoken_enroll(request, action):
    ...
    if ttype and ttype.lower() == WebAuthnTokenClass.get_class_type():
        ...
        authenticator_resident_key = (
            list(authenticator_resident_key_levels)[0]
        ...
        request.all_data[WEBAUTHNACTION.AUTHENTICATOR_RESIDENT_KEY] = (
            authenticator_resident_key
        )
        request.all_data[
            f"{WebAuthnTokenClass.get_class_type()}_{ACTION.CHALLENGETEXT}"
...
def webauthntoken_allowed(request, action):
    ...
    if ttype and ttype.lower() == WebAuthnTokenClass.get_class_type() and reg_data:
        ...
        if allowed_aaguids and aaguid not in [
            ...
            log.warning(
                f"The WebAuthn token {serial} is not allowed to be registered due to policy restriction {WEBAUTHNACTION.AUTHENTICATOR_SELECTION_LIST}"
            )
            raise PolicyError(
                "The WebAuthn token is not allowed to be registered due to a policy restriction."
            )
    ...
    cert_info = (
        {
            ...
            "attestation_serial": f"{attestation_cert.get_serial_number()}",
    ...
    if ttype and ttype.lower() == "certificate":
        ...
        if REQUIRE_ACTIONS.REQUIRE_AND_VERIFY in list(require_att):
            if not request.all_data.get("attestation"):
                ...
                log.warning(
                    f"The request is missing an attestation certificate. {require_att}"
                )
                raise PolicyError(
