 ...
 *
 */
myApp.controller("auditController", [
  "AuditFactory",
  "$scope",
  "$rootScope",
  "$stateParams",
  ...
  function (
    ...
    $scope.getParams = function () {
      ...
      $scope.params.edumfa_server = "*" + ($scope.serverFilter || "") + "*";
      ...
      if (
        ...
        AuditFactory.get($scope.params, function (data) {
          ...
          angular.forEach(
            ...
            function (auditentry, key) {
              if ($scope.auditdata.auditdata[key].policies != null) {
                var polname_list =
                  $scope.auditdata.auditdata[key].policies.split(",");
                // Duplicates in a repeater are not allowed!
                var uniquePolnameList = [];
                angular.forEach(polname_list, function (pol, i) {
                  if (uniquePolnameList.includes(pol) === false)
                    uniquePolnameList.push(pol);
                });
