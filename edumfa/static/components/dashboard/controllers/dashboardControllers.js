...
myApp.controller("dashboardController", [
  ...
  function (
    ...
    $scope.get_total_token_number = function () {
      ...
      TokenFactory.getTokensNoCancel(
        function (data) {
          if (data) {
    ...
    $scope.getAuthentication = function () {
      $scope.authentications = { success: 0, fail: 0 };
      AuditFactory.get(
        { timelimit: "1d", action: "*validate/*check", success: "1" },
        function (data) {
          $scope.authentications.success = data.result.value.count;
        },
      );
      AuditFactory.get(
        { timelimit: "1d", action: "*validate/*check", success: "0" },
        function (data) {
          ...
          angular.forEach(data.result.value.auditdata, function (auditentry) {
            if (auditentry.user) {
              ...
              if (
                !$scope.authentications.users[
                  auditentry.user + "-" + auditentry.realm
                ]
              ) {
                ...
                $scope.authentications.users[
                  ...
                  realm: auditentry.realm,
                  fails: 1,
                  latestError: auditentry.date,
                };
              } else {
          ...
          $scope.authentications.users = Object.values(
            $scope.authentications.users,
          );
    ...
    $scope.getAdministration = function () {
      ...
      angular.forEach(
        ["system", "resolver", "realm", "policy", "event"],
        function (adminaction) {
          AuditFactory.get(
            { timelimit: "1d", action: "POST /" + adminaction + "*" },
            function (data) {
              angular.forEach(
                data.result.value.auditdata,
                function (auditentry) {
