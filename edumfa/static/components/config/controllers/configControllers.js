 ...
 *
 */
myApp.controller("policyListController", [
  "$scope",
  "$stateParams",
  "$location",
  "ConfigFactory",
  function ($scope, $stateParams, $location, ConfigFactory) {
...
myApp.controller("policyDetailsController", [
  "$scope",
  "$stateParams",
  "ConfigFactory",
  "$state",
  ...
  function (
          ...
          if (value) {
            ...
            switch (vtype) {
              ...
              case "str":
                ...
                else $scope.params.action.push(key + "=" + aval);
                break;
              default:
    ...
    $scope.checkOpenGroup = function (action, pattern) {
      ...
      let re = RegExp(pat, "i");
      return (
        ($scope.actionCheckBox[action.name] || !$scope.onlySelectedVisible) &&
...
myApp.controller("tokenConfigController", [
  "$scope",
  "$location",
  "$rootScope",
  "$state",
  ...
  function (
    ...
    $scope.loadSystemConfig = function () {
      ConfigFactory.loadSystemConfig(function (data) {
        ...
        // SMS
        $scope.form["sms.Provider"] =
          $scope.form["sms.Provider"] || $scope.defaultSMSProvider;
        // Email
        ...
        $scope.form["totp.useTimeShift"] = $scope.isChecked(
          $scope.form["totp.useTimeShift"],
        );
        $scope.form["email.tls"] = $scope.isChecked($scope.form["email.tls"]);
        $scope.form["remote.verify_ssl_certificate"] = $scope.isChecked(
          $scope.form["remote.verify_ssl_certificate"],
        );
    ...
    $scope.saveTokenConfig = function () {
      ...
      var save_params = {};
      angular.forEach($scope.form, function (value, key) {
        if (value != $scope.original_params[key]) save_params[key] = value;
      });
...
myApp.controller("configController", [
  "$scope",
  "$location",
  "$rootScope",
  "$state",
  ...
  function (
    ...
    $scope.editResolver = function (resolvername, r_type) {
      ...
      $state.go("config.resolvers.edit" + r_type, {
    ...
    $scope.getSystemConfig = function () {
      ConfigFactory.getSystemConfig(function (data) {
        ...
        $scope.params.no_auth_counter = $scope.isChecked(
          $scope.params.no_auth_counter,
        );
        $scope.params.DisableAutoChallengeJanitor = $scope.isChecked(
          $scope.params.DisableAutoChallengeJanitor,
        );
        $scope.params.DisableClientTracking = $scope.isChecked(
          $scope.params.DisableClientTracking,
        );
        $scope.params["PrependPin.type"] = "public";
        $scope.params.splitAtSign = $scope.isChecked($scope.params.splitAtSign);
        $scope.params.IncFailCountOnFalsePin = $scope.isChecked(
          $scope.params.IncFailCountOnFalsePin,
        );
        $scope.params.ReturnSamlAttributes = $scope.isChecked(
          $scope.params.ReturnSamlAttributes,
        );
        $scope.params.ReturnSamlAttributesOnFail = $scope.isChecked(
          $scope.params.ReturnSamlAttributesOnFail,
        );
        $scope.params.AutoResync = $scope.isChecked($scope.params.AutoResync);
        $scope.params.UiLoginDisplayHelpButton = $scope.isChecked(
          $scope.params.UiLoginDisplayHelpButton,
        );
        $scope.params.UiLoginDisplayRealmBox = $scope.isChecked(
          $scope.params.UiLoginDisplayRealmBox,
        );
...
myApp.controller("PasswdResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
  function ($scope, ConfigFactory, $state, $stateParams) {
    $scope.params = {
      type: "passwdresolver",
      fileName: "/etc/passwd",
    };
...
myApp.controller("hostsResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
  function ($scope, ConfigFactory, $state, $stateParams) {
    ...
    $scope.setMachineResolver = function () {
      ConfigFactory.setMachineResolver(
        $scope.resolvername,
        $scope.params,
        function (data) {
          $scope.set_result = data.result.value;
...
myApp.controller("CAConnectorController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$rootScope",
  "$location",
  function ($scope, ConfigFactory, $state, $rootScope, $location) {
...
myApp.controller("LocalCAConnectorController", [
  "$scope",
  "$stateParams",
  "ConfigFactory",
  "$state",
  function ($scope, $stateParams, ConfigFactory, $state) {
    $scope.params = {
      type: "local",
    };
...
myApp.controller("MicrosoftCAConnectorController", [
  "$scope",
  "$stateParams",
  "ConfigFactory",
  "$state",
  function ($scope, $stateParams, ConfigFactory, $state) {
    ...
    $scope.getCASpecificOptions = function (catype) {
      ConfigFactory.getCASpecificOptions(
        catype,
        $scope.params,
        function (data) {
...
myApp.controller("machineResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$rootScope",
  ...
  function ($scope, ConfigFactory, $state, $rootScope, $location) {
    ...
    $scope.editResolver = function (resolvername, r_type) {
      ...
      $state.go("config.mresolvers.edit" + r_type, {
        resolvername: resolvername,
      });
...
myApp.controller("LdapResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
  ...
  function (
    ...
    $scope.tls_version_options = [
      { value: "3", name: "TLS v1.0" },
...
myApp.controller("ScimResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
...
myApp.controller("SqlResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
  ...
  function (
    ...
    $scope.presetWordpress = function () {
      ...
      $scope.params.Map =
        '{ "userid" : "ID", "username": "user_login", "email" : "user_email", "givenname" : "display_name", "password" : "user_pass" }';
    };

    $scope.presetOTRS = function () {
      $scope.params.Table = "users";
      $scope.params.Map =
        '{ "userid" : "id", "username": "login", "givenname" : "first_name", "surname" : "last_name", "password" : "pw" }';
    };

    $scope.presetTine = function () {
      $scope.params.Table = "tine20_accounts";
      $scope.params.Map =
        '{ "userid" : "id", "username": "login_name", "email" : "email", "givenname" : "first_name", "surname" : "last_name", "password" : "password" }';
    };

    $scope.presetOwncloud = function () {
      $scope.params.Table = "oc_users";
      $scope.params.Map =
        '{ "userid" : "uid", "username": "uid", "givenname" : "displayname", "password" : "password" }';
    };

    $scope.presetTypo3 = function () {
      $scope.params.Table = "be_users";
      $scope.params.Map =
        '{ "userid" : "uid", "username": "username", "givenname" : "realName", "password" : "password", "email": "email" }';
    };

    $scope.presetDrupal = function () {
      $scope.params.Table = "user";
      $scope.params.Map =
        '{"userid": "uid", "username": "name", "email"' +
        ': "mail", "password": "pass" }';
    };
...
myApp.controller("HTTPResolverController", [
  "$scope",
  "ConfigFactory",
  "$state",
  "$stateParams",
  ...
  function ($scope, ConfigFactory, $state, $stateParams, inform) {
    ...
    $scope.$watch("params.hasSpecialErrorHandler;", function (incomingValue) {
