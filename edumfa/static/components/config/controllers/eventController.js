  ...
  function ($scope, $stateParams, ConfigFactory, $state) {
    ...
    $scope.getEvent = function () {
      ConfigFactory.getEvent($scope.eventid, function (event) {
        ...
        ConfigFactory.getHandlerActions(
          event.result.value[0].handlermodule,
          function (actions) {
            ...
            angular.forEach($scope.opts, function (value, opt) {
              ...
              if (
                $scope.handlerOptions[$scope.form.action][opt] &&
                $scope.handlerOptions[$scope.form.action][opt].type ===
                  "bool" &&
                isTrue(value)
              ) {
                $scope.opts[opt] = true;
    ...
    $scope.getHandlerPositions = function () {
      ConfigFactory.getHandlerPositions(
        $scope.form.handlermodule,
        function (positions) {
    ...
    $scope.getHandlerConditions = function () {
      ...
      ConfigFactory.getHandlerConditions(
        $scope.form.handlermodule,
        function (conditions) {
    ...
    $scope.checkOpenConditionGroup = function (
      condition,
      conditionname,
      pattern,
    ) {
      ...
      let re = RegExp(pat, "i");
      return (
        ($scope.conditionCheckBox[conditionname] ||
