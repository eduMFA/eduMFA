 ...
 *
 */
myApp.directive("tokenDataEdit", [
  "AuthFactory",
  "instanceUrl",
  "versioningSuffixProvider",
  function (AuthFactory, instanceUrl, versioningSuffixProvider) {
    return {
      scope: {
        text: "@",
        buttonText: "@",
      ...
      link: function (scope, element, attr) {
        ...
        scope.loadUsers = function ($viewValue) {
          if (
            ...
            (!$viewValue || $viewValue == "*" || !scope.newUserObject.toggle)
          ) {
          ...
          return $http({
            ...
            url:
              ...
              "*" +
              "&realm=" +
              scope.newUserObject.realm,
            headers: { Authorization: auth_token },
          }).then(function ($response) {
            scope.newUserObject.loadedUsers = $response.data.result.value.map(
              function (item) {
    ...
    return {
      ...
      link: function (scope, element, attr) {
        ...
        scope.loadSerials = function ($viewValue) {
          if (
            $rootScope.search_on_enter &&
            (!$viewValue || $viewValue == "*" || !scope.newTokenObject.toggle)
          ) {
            // only use existing result if any, and if search_on_enter policy is active
          ...
          return $http({
            ...
            url: tokenUrl + "/",
            headers: { Authorization: auth_token },
            params: { assigned: "False", serial: "*" + $viewValue + "*" },
          }).then(function ($response) {
            scope.newTokenObject.loadedSerials =
              $response.data.result.value.tokens.map(function (item) {
...
myApp.directive("csvDownload", [
  "AuthFactory",
  "$http",
  "instanceUrl",
  "versioningSuffixProvider",
  function (AuthFactory, $http, instanceUrl, versioningSuffixProvider) {
    return {
      restrict: "E",
      templateUrl:
        instanceUrl +
...
myApp.directive("piPolicyConditions", [
  "instanceUrl",
  "versioningSuffixProvider",
  function (instanceUrl, versioningSuffixProvider) {
