 ...
 */
angular
  .module("eduMfaAuth", ["eduMfaApp.errorMessage"])
  .factory("AuthFactory", [
    "inform",
    "gettextCatalog",
    "$state",
    function (inform, gettextCatalog, $state) {
      ...
      return {
        ...
        authError: function (error) {
          ...
          if (error === null) {
            console.warn(
              "No error object available, maybe the request was aborted?",
            );
            return;
          }
          if (typeof error === "string") {
            inform.add(
              gettextCatalog.getString(
                "Failed to get a valid JSON response from the eduMFA server.",
              ),
...
angular.module("eduMfaAuth").factory("PollingAuthFactory", [
  "$http",
  "authUrl",
