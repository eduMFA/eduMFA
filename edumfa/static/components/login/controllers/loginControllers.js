...
angular.module("eduMfaApp").controller("mainController", [
  "Idle",
  "$scope",
  "$sce",
  "$http",
  ...
  function (
    ...
    obj = angular.element(
      document.querySelector("#EDUMFA_TRANSLATION_WARNING"),
    );
    ...
    $scope.welcomeStep = 0;
    // customization
    $scope.piCustomMenuFile = angular
      .element(document.querySelector("#CUSTOM_MENU"))
      .val();
    $scope.piCustomBaselineFile = angular
      .element(document.querySelector("#CUSTOM_BASELINE"))
      .val();
    ...
    $scope.confirmDelete = function (delete_function, identifier) {
      ...
      if (!$scope.confirmDeleteObj.question) {
        $scope.confirmDeleteObj.question = gettextCatalog.getString(
      ...
      if ($scope.deletion_confirmation) {
        ...
        $scope.confirmDeleteObj.delete_function(
          $scope.confirmDeleteObj.identifier,
        );
      }
    };
    ...
    */
    ...
    $scope.login = { username: $scope.remoteUser, password: "", realm: "" };
    $scope.transactionid = "";
    AuthFactory.setUser();
          ...
          function (response) {
            ...
            if (data.result.value && data.result.value.token) {
              ...
              if (multi_challenge.length > 1) {
                $scope.challenge_message = gettextCatalog.getString(
              ...
              for (var i = 0; i < multi_challenge.length; i++) {
                if (multi_challenge.length > 1) {
                  $scope.challenge_message =
                    $scope.challenge_message + " " + multi_challenge[i].serial;
                }
                ...
                if (challenge !== null) {
                  ...
                  if (attributes && attributes.webAuthnSignRequest) {
                    $scope.webAuthnSignRequests.push(
                  ...
                  if (
                    challenge.client_mode &&
                    challenge.client_mode === "poll"
                  ) {
                    $scope.polling = true;
                  }
                }
    ...
    $scope.check_authentication = function () {
      ...
      $http
        .get(validateUrl + "/polltransaction", {
          params: {
            transaction_id: $scope.transactionid,
          },
        })
        .then(
          ...
          function (response) {
            ...
            console.warn(
              "Polling for transactions returned an error: " + response.data,
            );
          },
        );
    };
    ...
    $scope.do_login_stuff = function (data) {
      ...
      if (data.detail) {
        ...
        if ($scope.next_pin_change && !$scope.pin_change_serial) {
          inform.add(
            gettextCatalog.getString("Your OTP pin expires on ") +
      ...
      $scope.subscription_state_push =
        data.result.value.subscription_status_push;
      ...
      if ($scope.loggedInUser.role === "admin") {
        ...
        $scope.resetWelcome();
        if (
          ($scope.welcomeStep < 4 && !$scope.hide_welcome) ||
          $scope.welcomeStep < 5
        ) {
          // We did not walk through the welcome dialog, yet.
          if (
            ($scope.subscription_state === 0 && !$scope.hide_welcome) ||
        ...
        $scope.class_subscription_expired =
          "subscriptionExpired" + $random_number;
        // Show info about unsupported privacyIDEA Authenticator App and push
        if ($scope.subscription_state_push === 1 && !$scope.hide_welcome) {
          // no subscription at all
          inform.add(
            gettextCatalog.getString(
              "You are using a certain amount of Push tokens. " +
        ...
        if ($scope.subscription_state_push === 2) {
          ...
          inform.add(
            gettextCatalog.getString(
              "Your subscription for the unsupported privacyIDEA Authenticator" +
                " has expired.",
            ),
    ...
    $scope.logout = function () {
      ...
      $scope.loggedInUser = {};
      $scope.eduMFAVersionNumber = null;
      $scope.logoutWarning = false;
      $scope.myCountdown = "";
      $scope.resetWelcome();
    ...
    $scope.nextWelcome = function () {
      $scope.welcomeStep += 1;
      if (
        ($scope.subscription_state === 0 && $scope.welcomeStep === 4) ||
    ...
    $scope.createDefaultRealm = function () {
      ...
      ConfigFactory.setResolver("deflocal", resolver_params, function (data) {
        if (data.result.value) {
          // The resolver is created, we can create the realm
          ConfigFactory.setRealm("defrealm", realm_params, function (data) {
            if (data.result.value) {
              inform.add(
                gettextCatalog.getString("Realm " + "defrealm created."),
                { type: "info" },
              );
            }
          });
...
angular.module("eduMfaApp").controller("pinChangeController", [
  "Idle",
  "$scope",
  "$http",
  "$location",
