 ...
 *
 */
myApp.controller("tokenController", [
  "TokenFactory",
  "ConfigFactory",
  "$scope",
  "$location",
  ...
  function (
    ...
    $scope.get = function (live_search) {
      if (
        ...
        $scope.params.rollout_state =
          "*" + ($scope.rolloutStateFilter || "") + "*";
...
myApp.controller("tokenAssignController", [
  "$scope",
  "TokenFactory",
  "$stateParams",
  "AuthFactory",
  "UserFactory",
  "$state",
  function tokenAssignController(
    $scope,
    TokenFactory,
    $stateParams,
    AuthFactory,
...
myApp.controller("tokenEnrollController", [
  "$scope",
  "TokenFactory",
  "$timeout",
  "$stateParams",
  ...
  function tokenEnrollController(
    $scope,
    TokenFactory,
    $timeout,
    $stateParams,
    ...
    $scope.loadAvailableServiceIDs = function () {
      ConfigFactory.getServiceid("", function (data) {
        ...
        angular.forEach(serviceids, function (serviceid_data, name) {
          $scope.formInit.service_ids[name] =
            name + ": " + serviceid_data.description;
        });
      });
    };
    ...
    $scope.setVascoSerial = function () {
      if ($scope.form.otpkey.length === 496) {
        ...
        for (var i = 0; i < vasco_hex.length; i += 2)
          vasco_otpstr += String.fromCharCode(
            parseInt(vasco_hex.substr(i, 2), 16),
          );
    ...
    $scope.changeTokenType = function () {
      ...
      if ($scope.form.type === "hotp") {
        ...
        $scope.form.timeStep = parseInt(
          $scope.systemDefault["totp.timeStep"] || "30",
        );
        $scope.form.useTimeShift = "default";
      } else if ($scope.form.type === "daypassword") {
        // preset DayPassword hashlib
        $scope.form.hashlib =
          $scope.systemDefault["daypassword.hashlib"] || "sha1";
        $scope.form.timeStep = parseInt(
          $scope.systemDefault["daypassword.timeStep"] || "60",
        );
      }
      ...
      if ($scope.form.type === "indexedsecret") {
        ...
        if ($scope.tokensettings.indexedsecret.preset_attribute) {
          ...
          UserFactory.getUsers(params, function (data) {
            ...
            $scope.form.otpkey =
              userObject[$scope.tokensettings.indexedsecret.preset_attribute];
          });
        }
      }
    };
    ...
    $scope.setTwostepEnrollmentDefault = function () {
      $scope.form["2stepinit"] = $scope.checkRight(
        $scope.form.type + "_2step=force",
      );
    };
      ...
      ConfigFactory.getRealms(function (data) {
        ...
        if ($stateParams.username) {
          ...
          UserFactory.getUsers(
            { realm: $scope.newUser.realm, username: $scope.newUser.user },
            function (data) {
    ...
    TokenFactory.getEnrollTokens(function (data) {
      ...
      if (
        !$scope.formInit.tokenTypes.hasOwnProperty($scope.default_tokentype)
      ) {
        ...
        if ($scope.default_tokentype != "hotp") {
          inform.add(
            `Your user has no permission to enroll the default tokentype '${$scope.default_tokentype}'. Using '${tkey}' instead.`,
    ...
    $scope.callback = function (data) {
      ...
      if ($scope.enrolledToken.otps) {
        ...
        $scope.otp_rows = Object.keys($scope.enrolledToken.otps).slice(
          0,
          $scope.otp_row_count,
        );
      } else {
      ...
      if ($scope.enrolledToken.pkcs12) {
        ...
        $scope.pkcs12Blob = (window.URL || window.webkitURL).createObjectURL(
          blob,
        );
      }
      if ($scope.enrolledToken.u2fRegisterRequest) {
        ...
        $scope.register_fido(
          $scope.enrolledToken.u2fRegisterRequest,
          U2fFactory,
          $scope.U2FToken,
        );
      }
    ...
    $scope.enrollToken = function () {
      ...
      $scope.form.validity_period_start = date_object_to_string(
        $scope.form.validity_period_start,
      );
      $scope.form.validity_period_end = date_object_to_string(
        $scope.form.validity_period_end,
      );
      TokenFactory.enroll(
        $scope.newUser,
        $scope.form,
    ...
    $scope.pollTokenInfo = function () {
      TokenFactory.getTokenForSerial(
        $scope.enrolledToken.serial,
        function (data) {
          $scope.enrolledToken.rollout_state =
            data.result.value.tokens[0].rollout_state;
    ...
    $scope.register_fido = function (registerRequest, Factory, token) {
      ...
      Factory.register_request(registerRequest, function (params) {
        ...
        TokenFactory.enroll($scope.newUser, params, function (response) {
          $scope.click_wait = false;
          token.subject = (
            response.detail.u2fRegisterResponse ||
            response.detail.webAuthnRegisterResponse
          ).subject;
    ...
    // get the list of configured eduMFA server identifiers
    $scope.getEduMfaServers = function () {
    ...
    $scope.openCertificateWindow = function () {
      ...
      $http
        .post(instanceUrl + "/certificate", params)
...
myApp.controller("tokenImportController", [
  "$scope",
  "Upload",
  "AuthFactory",
  "tokenUrl",
  ...
  function (
    ...
    $scope.verify_pskc_opts = {
      ...
      check_fail_soft: gettextCatalog.getString(
        "Skip tokens that can not be verified",
      ),
      check_fail_hard: gettextCatalog.getString(
