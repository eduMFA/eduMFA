...
myApp.controller("tokenDetailController", [
  "$scope",
  "TokenFactory",
  "UserFactory",
  "$stateParams",
  ...
  function (
    ...
    $scope.get = function () {
      TokenFactory.getTokenForSerial($scope.tokenSerial, function (data) {
        ...
        $scope.validity_period_start = string_to_date_object(
          $scope.token.info.validity_period_start,
        );
        $scope.validity_period_end = string_to_date_object(
          $scope.token.info.validity_period_end,
        );
    ...
    $scope.sendVerifyResponse = function () {
      ...
      TokenFactory.enroll($scope.newUser, params, function (data) {
        ...
        if (data.result.value === true) {
          inform.add(
            gettextCatalog.getString("Enrollment successfully verified."),
            { type: "info", ttl: 10000 },
          );
        } else {
          inform.add(
            gettextCatalog.getString("Enrollment verification failed."),
    ...
    if ($scope.loggedInUser.role === "admin") {
      ...
      $scope.detachMachineToken = function (application, mtid) {
        MachineFactory.detachTokenMachine(
          { serial: $scope.tokenSerial, application: application, mtid: mtid },
          function (data) {
            $scope.getMachines();
          },
        );
    ...
    $scope.rolloverTokenAllowed = function (token) {
      if (typeof token != "undefined") {
        if (
          $scope.checkEnroll() &&
          token.tokentype in $scope.token_rollover &&
