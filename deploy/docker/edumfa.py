...
def _getenv(key, default="") -> str:
    ...
    :raises Exception: If the variable is not set and there is no default
        value.
    """
    value = getenv(key)
    value = value if value else default
    if not value:
        raise Exception(f"Environment variable '{key}' not set! Can't start...")
    else:
        return value
