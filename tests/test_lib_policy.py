...
from .base import (
    PWFILE as FILE_PASSWORDS,
    PWFILE2 as FILE_PASSWD,
    FakeAudit,
    FakeFlaskG,
    MyTestCase,
)
...
class PolicyTestCase(MyTestCase):
    ...
    def test_01_create_simple_policy(self):
        ...
        p = set_policy(
            name="pol4", action="enroll, init, disable , enable", scope="admin"
        )
        self.assertTrue(p > 0)
    ...
    def test_24_challenge_text(self):
        ...
        set_policy(
            "chaltext",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT!s}=Wo du wolle?",
        )
    ...
    def test_25_get_action_values(self):
        ...
        set_policy(
            "act1", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=userstore", priority=1
        )
        set_policy(
            "act2", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=userstore", priority=1
        )
        set_policy(
            "act3", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=none", priority=3
        )
    ...
    def test_26_match_policies_user_object(self):
        ...
        set_policy(
            "act3", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=none", realm="realm1"
        )
        set_policy(
            "act4",
            scope=SCOPE.AUTH,
            action=f"{ACTION.OTPPIN!s}=none",
            user="nönäscii",
            realm="realm1",
        )
    ...
    def test_27_reload_policies(self):
        ...
        set_policy(
            "act1", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=userstore", priority=2
        )
        self.assertEqual(policy_object.match_policies()[0]["priority"], 2)
        # Add a second policy, check
        set_policy(
            "act2", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=none", priority=3
        )
        self.assertEqual(
