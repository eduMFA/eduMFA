...
from edumfa.models import Policy, PolicyCondition, Token, db
...
class PolicyTestCase(MyTestCase):
    ...
    def test_01_create_simple_policy(self):
        ...
        policies = P.match_policies(scope=SCOPE.ADMIN, action=ACTION.DISABLE)
        self.assertTrue(len(policies) == 1, len(policies))
    ...
    def test_02_update_policies(self):
        ...
        p = set_policy(
            name="pol2a", action="tokentype=TOTP", scope=SCOPE.AUTHZ, realm=self.realm2
        )
        self.assertTrue(p > 0)
        ...
        self.assertTrue(len(policies) == 1, f"{len(policies)}")
    ...
    def test_07_client_policies(self):
        ...
        set_policy(name="pol1", scope=SCOPE.USER, client="172.16.0.3, 172.16.0.4/24")
        set_policy(
            name="pol2", scope=SCOPE.AUTHZ, client="192.168.0.0/16, -192.168.1.1"
        )
        ...
        set_policy(name="pol1", scope=SCOPE.USER)
        self.assertEqual("172.16.0.3, 172.16.0.4/24", policy.client)
        # Set empty string removes client
        set_policy(name="pol1", scope=SCOPE.USER, client="")
        self.assertEqual("", policy.client)

    def test_08_user_policies(self):
        set_policy(name="pol1", scope=SCOPE.USER, user="*")
    ...
    def test_08a_adminuser_policies(self):
        ...
        set_policy(
            name="pol2", scope=SCOPE.ADMIN, adminuser="admin, root, user1", user="*"
        )
    ...
    def test_09_realm_resolver_policy(self):
        set_policy(name="pol1", scope=SCOPE.USER, realm=self.realm1)
        set_policy(
            name="pol2",
            scope=SCOPE.AUTHZ,
            realm=self.realm1,
    ...
    def test_10_action_policies(self):
        set_policy(
            name="pol1",
            scope=SCOPE.ADMIN,
            action=f"enrollHOTP, {ACTION.ENABLE}, {ACTION.DISABLE}",
        )
        set_policy(
            name="pol2",
            scope=SCOPE.AUTH,
            action=f"{ACTION.RESETALLTOKENS}, {ACTION.OTPPIN}=1",
        )
    ...
    def test_14_fail_unique_policies(self):
        ...
        set_policy(
            name="email1", scope=SCOPE.AUTH, action=f"{EMAILACTION.EMAILTEXT}=text 1"
        )
        set_policy(
            name="email2", scope=SCOPE.AUTH, action=f"{EMAILACTION.EMAILTEXT}=text 2"
        )
    ...
    def test_16_admin_realm(self):
        ...
        set_policy(
            name="tokenEnroll", scope=SCOPE.ADMIN, adminrealm="", action="enrollYUBIKEY"
        )
        policy = Policy.query.filter_by(name="tokenEnroll").first()
    ...
    def test_21_check_all_resolver(self):
        ...
        user = User(login="cornelius", realm="test_realm")
        # The user, that is created, is cornelius.reso1@realm1
        user_str = f"{user}"
        self.assertEqual(user_str, "<cornelius.reso1@test_realm>")
    ...
    def test_23_priorities(self):
        ...
        set_policy(
            name="email1",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILTEXT}=text 1",
            priority=4,
        )
        set_policy(
            name="email2",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILTEXT}=text 2",
            priority=1,
        )
        set_policy(
            name="email3",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILTEXT}=text 3",
            priority=77,
        )
        ...
        set_policy(
            name="email4",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILTEXT}=text 4",
            priority=77,
        )
    ...
    def test_23_priorities_equal_actions(self):
        ...
        self.assertEqual(
            ...
            list(
                P.get_action_values(
                    scope=SCOPE.AUTH, action=EMAILACTION.EMAILTEXT
                ).keys()
            ),
        )
        ...
        self.assertEqual(
            ...
            list(
                P.get_action_values(
                    scope=SCOPE.AUTH, action=EMAILACTION.EMAILTEXT, unique=True
                ).keys()
            ),
        )
    ...
    def test_25_get_action_values(self):
        ...
        set_policy(
            "act1", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=userstore", priority=1
        )
        set_policy(
            "act2", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=userstore", priority=1
        )
        set_policy(
            "act3", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=userstore", priority=3
        )
    ...
    def test_26_match_policies_user_object(self):
        ...
        set_policy(
            "act3", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=none", realm="realm1"
        )
        set_policy(
            "act4",
            scope=SCOPE.AUTH,
            action=f"{ACTION.OTPPIN}=none",
            user="nönäscii",
            realm="realm1",
        )
        ...
        self.assertSetEqual(
            set(), set(p["name"] for p in P.match_policies(user_object=whoopsie))
        )
        self.assertEqual(
            P.get_action_values(
        ...
        self.assertSetEqual(
            set(), set(p["name"] for p in P.match_policies(user_object=User()))
        )
        self.assertEqual(
            {},
            P.get_action_values(
        ...
        self.assertSetEqual(
            {"act5"}, set(p["name"] for p in P.match_policies(user_object=User()))
        )
        self.assertEqual(
            {"none": ["act5"]},
            P.get_action_values(
    ...
    def test_27_reload_policies(self):
        ...
        set_policy(
            "act1", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=userstore", priority=2
        )
        ...
        self.assertSetEqual(
            {"act1", "act2"}, {p["name"] for p in policy_object.match_policies()}
        )
        # Delete a policy, check
        delete_policy("act1")
        self.assertSetEqual(
            {"act2"}, {p["name"] for p in policy_object.match_policies()}
        )
        delete_policy("act2")
    ...
    def test_28_conditions(self):
        ...
        self.assertIsNone(
            PolicyCondition.query.filter_by(
                section=ConditionSection.USERINFO,
                Key="type",
        ...
        self.assertRaises(
            ...
            "invalid_policy",
            conditions=[
                (ConditionSection.USERINFO, "type", Comparators.EQUALS, "verysecure")
            ],
        )
    ...
    def test_29_filter_by_conditions(self):
        ...
        self.assertSetEqual(
            {"notverysecure"}, _names(P.match_policies(user_object=user3))
        )
        ...
        self.assertSetEqual(
            {"extremelysecure"}, _names(P.match_policies(user_object=user1))
        )
    ...
    def test_30_filter_by_conditions_errors(self):
        ...
        set_policy(
            ...
            action=f"{ACTION.OTPPIN}=userstore",
            conditions=[
                (ConditionSection.USERINFO, "number", Comparators.CONTAINS, "b", True)
            ],
        )
        with self.assertRaisesRegex(PolicyError, r".*Invalid comparison.*"):
    ...
    def test_31_match_edumfanode(self):
        ...
        set_policy(
            ...
            edumfanode="edumfanode2, edumfanode1",
        )
    ...
    def test_31_filter_by_conditions_tokeninfo(self):
        ...
        self.assertSetEqual(
            {"setpin_pol"}, _names(P.match_policies(user_object=user1, serial=serial))
        )
        ...
        self.assertSetEqual(
            set(), _names(P.match_policies(user_object=user1, serial=serial))
        )
    ...
    def test_32_filter_by_conditions_token(self):
        ...
        set_policy(
            ...
            action=ACTION.SETPIN,
            conditions=[
                (ConditionSection.TOKEN, "tokentype", Comparators.EQUALS, "hotp", False)
            ],
        )
        ...
        set_policy(
            "setpin_pol",
            conditions=[
                (ConditionSection.TOKEN, "tokentype", Comparators.EQUALS, "hotp", True)
            ],
        )
        ...
        set_policy(
            "setpin_pol",
            conditions=[
                (ConditionSection.TOKEN, "tokentype", Comparators.EQUALS, "Spass", True)
            ],
        )
        self.assertSetEqual(
            set(), _names(P.match_policies(user_object=user1, serial=serial))
        ...
        set_policy(
            "setpin_pol",
            conditions=[
                (ConditionSection.TOKEN, "random", Comparators.SMALLER, "100", True)
            ],
        )
        self.assertRaises(
    ...
    def test_40_disable_policy_client_remains(self):
        ...
        test_ip = "1.2.3.4"
        set_policy(
            pname, scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN}=none", client=test_ip
        )
        p = PolicyClass()
    ...
    def test_44_filter_by_condition_missing_data_error(self):
        ...
        error_message = (
            r"Policy 'policy' has a condition on the section 'tokeninfo' with key 'fixedpin', but a token "
            r"is unavailable!"
        )
    ...
    def test_48_condition_handle_missing_data_get_selection_dict(self):
        ...
        enum_members = {
            member.value for member in ConditionHandleMissingData.__members__.values()
        }
    ...
    def test_49_get_policy_condition_from_tuple(self):
        ...
        condition = PolicyClass.get_policy_condition_from_tuple(
            condition_tuple, "policy"
        )
        ...
        self.assertEqual(
            ConditionHandleMissingData.RAISE_ERROR, condition.handle_missing_data
        )
        ...
        self.assertTrue(condition.active)
        self.assertEqual(
            ConditionHandleMissingData.IS_TRUE, condition.handle_missing_data
        )
...
class PolicyConditionClassTestCase(MyTestCase):
    def test_01_init_success(self):
        ...
        condition = PolicyConditionClass(
            ConditionSection.USERINFO,
            "email",
            Comparators.MATCHES,
            ".*@example.com",
    ...
    def test_02_init_invalid_parameters(self):
        ...
        with self.assertRaises(ParameterError) as exception:
            PolicyConditionClass(
                "invalid", "email", Comparators.MATCHES, ".*@example.com", True
            )
        ...
        with self.assertRaises(ParameterError) as exception:
            ...
            self.assertEqual(
                "Key must be a non-empty string. Got '[\"email\"]' of type 'list' instead.",
                exception.exception.message,
            )
        ...
        with self.assertRaises(ParameterError) as exception:
            PolicyConditionClass(
                ConditionSection.USERINFO, "email", "random", ".*@example.com", True
            )
            self.assertEqual(
                "Unknown comparator 'random'.", exception.exception.message
            )

        # Invalid value
        with self.assertRaises(ParameterError) as exception:
            PolicyConditionClass(
                ConditionSection.USERINFO, "email", Comparators.MATCHES, False, True
            )
            self.assertIn(
                "Value must be a non-empty string.", exception.exception.message
            )
        ...
        with self.assertRaises(ParameterError) as exception:
            ...
            self.assertIn(
                "Unknown handle missing data value", exception.exception.message
            )
    ...
    def test_03_allow_invalid_parameters(self):
        ...
        with self.assertRaises(ParameterError) as exception:
            ...
            self.assertIn(
                "Invalid condition can not be activated", exception.exception.message
            )
            self.assertIn("Unknown section", exception.exception.message)
    ...
    def test_04_get_user_data(self):
        ...
        condition = PolicyConditionClass(
            section=ConditionSection.USERINFO,
            key="birthday",
            comparator=Comparators.MATCHES,
            value=".*May.*",
            active=True,
        )
    ...
    def test_05_get_token_data_object_not_available(self):
        condition = PolicyConditionClass(
            section=ConditionSection.TOKEN,
            key="tokentype",
            comparator=Comparators.EQUALS,
            value="hotp",
            active=True,
    ...
    def test_07_get_token_data_token_info(self):
        condition = PolicyConditionClass(
            section=ConditionSection.TOKENINFO,
            key="hashlib",
            comparator=Comparators.EQUALS,
            value="sha256",
            active=True,
    ...
    def test_08_get_request_header_data(self):
        condition = PolicyConditionClass(
            section=ConditionSection.HTTP_REQUEST_HEADER,
            key="User-Agent",
            comparator=Comparators.EQUALS,
            value="SpecialApp",
            active=True,
    ...
    def test_09_get_request_environment_data(self):
        condition = PolicyConditionClass(
            section=ConditionSection.HTTP_ENVIRONMENT,
            key="REQUEST_METHOD",
            comparator=Comparators.EQUALS,
            value="POST",
            active=True,
    ...
    def test_10_do_handle_missing_data_raise_error(self):
        ...
        # Token object not available
        error_message = (
            r"Policy 'test' has a condition on the section 'tokeninfo' with key 'hashlib', but a token is "
            r"unavailable!"
        )
        with self.assertRaisesRegex(PolicyError, error_message):
            condition._do_handle_missing_data(
                policy_name="test", missing="token", object_name="token"
            )

        # Key not available
        error_message = (
            r"Unknown tokeninfo key 'hashlib' referenced in condition of policy 'test'"
        )
        with self.assertRaisesRegex(PolicyError, error_message):
            condition._do_handle_missing_data(
                policy_name="test",
        ...
        error_message = (
            r"Policy 'test' has a condition on the section 'tokeninfo' with key 'hashlib', but some "
            r"required data is unavailable!"
        )
        with self.assertRaisesRegex(PolicyError, error_message):
            condition._do_handle_missing_data(
                policy_name="test", missing="user", object_name="token"
            )
    ...
    def test_13_do_handle_missing_data_invalid(self):
        ...
        self.assertFalse(
            ConditionHandleMissingData.RANDOM in ConditionHandleMissingData.__members__
        )
        error_message = (
            r"Unknown handle missing data random defined in condition of policy test."
        )
        condition = PolicyConditionClass(
            section=ConditionSection.TOKENINFO,
            key="hashlib",
            ...
            value="sha256",
            active=False,
            handle_missing_data=ConditionHandleMissingData.RANDOM,
            pass_if_inactive=True,
        )
        with self.assertRaisesRegex(PolicyError, error_message):
            condition._do_handle_missing_data(
...
class ConditionHandleMissingDataTestCase(MyTestCase):
    ...
    def test_02_get_from_value(self):
        # valid value
        enum = ConditionHandleMissingData.get_from_value(
            ConditionHandleMissingData.IS_TRUE.value
        )
...
class PolicyMatchTestCase(MyTestCase):
    def setUp(self):
        ...
        set_policy(
            name="pol2", action=f"{ACTION.TOKENTYPE}=HOTP", scope=SCOPE.AUTHZ, realm="*"
        )
        set_policy(
            name="pol2a",
            action=f"{ACTION.TOKENTYPE}=TOTP",
    ...
    def test_01_action_only(self):
        ...
        self.assertSetEqual(
            {"pol2", "pol2a", "pol3"}, set(g.audit_object.audit_data["policies"])
        )

        g.audit_object.audit_data = {}
        self.check_names(
            Match.action_only(g, SCOPE.AUTHZ, ACTION.TOKENTYPE).policies(),
            {"pol2", "pol2a"},
        )
        self.assertSetEqual(
            {"pol2", "pol2a"}, set(g.audit_object.audit_data["policies"])
        )
        g.audit_object.audit_data = {}
        self.assertEqual(
            Match.action_only(g, SCOPE.AUTHZ, ACTION.TOKENTYPE).action_values(
                unique=False
            ),
        ...
        g.audit_object.audit_data = {}
        self.check_names(
            Match.action_only(g, SCOPE.AUTHZ, ACTION.NODETAILSUCCESS).policies(), {}
        )
    ...
    def test_02_realm(self):
        ...
        g.audit_object.audit_data = {}
        self.check_names(
            Match.realm(g, SCOPE.AUTHZ, ACTION.TOKENTYPE, None).policies(),
            {"pol2", "pol2a"},
        )
        self.check_names(
            Match.realm(g, SCOPE.AUTHZ, ACTION.TOKENTYPE, "realm1").policies(), {"pol2"}
        )
        self.check_names(
            Match.realm(g, SCOPE.AUTHZ, ACTION.TOKENTYPE, "realm2").policies(),
            {"pol2", "pol2a"},
        )
        self.check_names(
            Match.realm(g, SCOPE.AUTHZ, ACTION.TOKENTYPE, "realm3").policies(), {"pol2"}
        )
        self.check_names(
            Match.realm(g, SCOPE.AUTHZ, ACTION.SERIAL, "realm1").policies(), {"pol3"}
        )
    ...
    def test_03_user(self):
        ...
        self.check_names(
            Match.user(g, SCOPE.USER, ACTION.AUDIT, Foobar()).policies(), {"pol1"}
        )
        self.check_names(Match.user(g, SCOPE.USER, ACTION.AUDIT, Baz()).policies(), {})
        self.check_names(
            Match.user(g, SCOPE.USER, ACTION.AUDIT, None).policies(), {"pol1"}
        )
    ...
    def test_04_admin(self):
        ...
        self.check_names(Match.admin(g, ACTION.ENABLE, None).policies(), {"pol4"})
        self.check_names(
            Match.admin(g, ACTION.ENABLE, User("cornelius", "realm2")).policies(),
            {"pol4"},
        )
        self.check_names(
            Match.admin(g, ACTION.ENABLE, User("cornelius", "realm1")).policies(), {}
        )
    ...
    def test_05_admin_or_user(self):
        ...
        g.logged_in_user = {"username": "superroot", "realm": "", "role": ROLE.ADMIN}
        self.check_names(
            Match.admin_or_user(g, ACTION.AUDIT, None).policies(), {"pol4"}
        )
        self.check_names(
            Match.admin_or_user(
                g, ACTION.AUDIT, User("cornelius", "realm2")
            ).policies(),
            {"pol4"},
        )
        self.check_names(
            Match.admin_or_user(
                g, ACTION.AUDIT, User("cornelius", "realm1")
            ).policies(),
            {},
        )
        ...
        self.check_names(
            Match.admin_or_user(g, ACTION.AUDIT, None).policies(), {"pol1"}
        )
        ...
        with self.assertRaises(MatchingError):
            self.check_names(
                Match.admin_or_user(
                    g, ACTION.ENABLE, User("cornelius", "realm1")
                ).policies(),
                {"pol4"},
            )
