...
class APIEventsTestCase(MyApiTestCase):
    ...
    def test_02_test_options(self):
        ...
        # delete event
        with self.app.test_request_context(
            f"/event/{ev1_id!s}", method="DELETE", headers={"Authorization": self.at}
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            result = res.json.get("result")
    ...
    def test_07_positions(self):
        ...
        # check the event
        with self.app.test_request_context(
            f"/event/{ev1_id!s}", method="GET", headers={"Authorization": self.at}
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            result = res.json.get("result")
