...
class TokenTestCase(MyTestCase):
    ...
    def test_15_init_token(self):
        ...
        self.assertRaises(
            ParameterError,
            init_token,
            {"otpkey": "1234", "type": "totp", "info": "[1,2,3,4]"},
        )
    ...
    def test_30_set_max_failcount(self):
        ...
        self.assertTrue(r == 1, r)
        self.assertTrue(
            tokenobject.token.maxfail == 112, f"{tokenobject.token.maxfail!s}"
        )
        remove_token(serial)
    ...
    def test_34_check_token_list(self):
        ...
        hotp_tokenobject.add_tokeninfo(
            "next_pin_change", f"{datetime.datetime(2019, 1, 7, 0, 0)!s}"
        )
        hotp_tokenobject.add_tokeninfo(
            "next_password_change", f"{datetime.datetime(2019, 1, 7, 0, 0)!s}"
        )
    ...
    def test_36_check_user_pass(self):
        ...
        self.assertFalse(r)
        self.assertTrue(
            reply.get("message") == "The user has no tokens " "assigned", f"{reply!s}"
        )
...
class PINChangeTestCase(MyTestCase):
    ...
    def test_00_create_realms(self):
        ...
        set_policy(
            "every10d", scope=SCOPE.ENROLL, action=f"{ACTION.CHANGE_PIN_EVERY!s}=10d"
        )
        # set policy for chalresp
        set_policy(
            "chalresp", scope=SCOPE.AUTH, action=f"{ACTION.CHALLENGERESPONSE!s}=hotp"
        )
    ...
    def test_01_successfully_change_pin(self):
        ...
        r, reply_dict = check_token_list(
            [tok, tok2],
            f"{newpin!s}{self.valid_otp_values[2]!s}",
            user=user_obj,
            options={"g": g},
        )
        self.assertTrue(r)
    ...
    def test_02_failed_change_pin(self):
        ...
        r, reply_dict = check_token_list(
            [tok, tok2],
            f"test{self.valid_otp_values[1]!s}",
            user=user_obj,
            options={"g": g},
        )
        self.assertFalse(r)
