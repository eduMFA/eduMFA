...
class DayPasswordTokenTestCase(MyTestCase):
    ...
    def test_27_get_default_settings(self):
        ...
        set_policy(
            "pol1",
            scope=SCOPE.USER,
            action="daypassword_hashlib=sha256,daypassword_timestep=60,daypassword_otplen=8",
        )
        ...
        set_policy(
            "pol1",
            scope=SCOPE.ADMIN,
            action="daypassword_hashlib=sha512,daypassword_timestep=60,daypassword_otplen=8",
        )
        g.policy_object = PolicyClass()
        p = DayPasswordTokenClass.get_default_settings(g, params)
        self.assertEqual(p.get("otplen"), "8")
