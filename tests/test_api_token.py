...
class API000TokenAdminRealmList(MyApiTestCase):
    ...
    def test_02_two_resolver_in_realm_policy_condition(self):
        ...
        with self.app.test_request_context(
            f"/token/disable/{t1.token.serial!s}",
            method="POST",
            data={},
            headers={"Authorization": self.at},
        ):
        ...
        with self.app.test_request_context(
            f"/token/disable/{t2.token.serial!s}",
            method="POST",
            data={},
            headers={"Authorization": self.at},
        ):
        ...
        with self.app.test_request_context(
            f"/token/enable/{t1.token.serial!s}",
            method="POST",
            data={},
            headers={"Authorization": self.at},
        ):
        ...
        with self.app.test_request_context(
            f"/token/enable/{t2.token.serial!s}",
            method="POST",
            data={},
            headers={"Authorization": self.at},
        ):
...
class APITokenTestCase(MyApiTestCase):
    ...
    def test_19_get_challenges(self):
        set_policy(
            "chalresp", scope=SCOPE.AUTHZ, action=f"{ACTION.CHALLENGERESPONSE!s}=hotp"
        )
    ...
    def test_32_set_random_pin(self):
        ...
        set_policy(
            "allowed_to_set_pin", scope=SCOPE.ADMIN, action=f"{ACTION.SETRANDOMPIN!s}"
        )
        # at least we need a otppinrandom policy (but not with length 0
        set_policy(
            "pinpolrandom", scope=SCOPE.ADMIN, action=f"{ACTION.OTPPINSETRANDOM!s}=0"
        )
        ...
        set_policy(
            "pinpolrandom", scope=SCOPE.ADMIN, action=f"{ACTION.OTPPINSETRANDOM!s}=10"
        )
        ...
        set_policy(
            "pinpolrandom2", scope=SCOPE.ADMIN, action=f"{ACTION.OTPPINSETRANDOM!s}=9"
        )
        ...
        set_policy(
            "pinpolrandom2",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.OTPPINSETRANDOM!s}=9",
            priority=1,
        )
        set_policy(
            "pinpolrandom",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.OTPPINSETRANDOM!s}=10",
            priority=2,
        )
    ...
    def test_40_init_verify_hotp_token(self):
        set_policy(
            "verify_toks1",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=hotp top",
        )
        set_policy(
            "verify_toks2",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=HOTP email",
        )
        set_policy(
            "require_description",
            scope=SCOPE.ENROLL,
    ...
    def test_41_init_verify_email_token(self):
        set_policy(
            "verify_toks1",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=email",
        )
    ...
    def test_42_init_verify_sms_token(self):
        set_policy(
            "verify_toks1",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=sms",
        )
        # Enroll an email token
        with self.app.test_request_context(
            "/token/init",
    ...
    def test_43_init_verify_index_token(self):
        set_policy(
            "verify_toks1",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=indexedsecret",
        )
...
class APIMSCACertTestCase(MyApiTestCase):
    ...
    def test_01_msca_certificate_pending_and_enrolled(self):
        with mock.patch.object(
            ...
            with self.app.test_request_context(
                f"/token/?serial={cert_tok.token.serial!s}",
                method="GET",
                headers={"Authorization": self.at},
            ):
                res = self.app.full_dispatch_request()
...
class APITokengroupTestCase(MyApiTestCase):
    ...
    def test_01_add_tokengroups(self):
        ...
        with self.app.test_request_context(
            f"/token/group/{serial!s}/gruppe1",
            method="POST",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
