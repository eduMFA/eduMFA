...
class APIMachinesTestCase(MyApiTestCase):
    ...
    def test_10_auth_items_ssh_rsa(self):
        ...
        with self.app.test_request_context(
            f"/machine/token/{self.serial2!s}/ssh/{mtid!s}",
            method="DELETE",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
        ...
        with self.app.test_request_context(
            ...
            self.assertTrue(
                passlib.hash.pbkdf2_sha512.verify(
                    "test903435", response.get("18")  # count = 18
                )
            )
            self.assertTrue(
                passlib.hash.pbkdf2_sha512.verify(
                    "test749439", response.get("34")  # count = 34
                )
            )
    ...
    def test_20_detach_ssh_key_any_token(self):
        ...
        with self.app.test_request_context(
            f"/machine/token/{serial!s}/any%20machine/no%20resolver/ssh",
            method="DELETE",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
    ...
    def test_30_detach_old_offline_token(self):
        ...
        with self.app.test_request_context(
            f"/machine/token/{serial!s}/192.168.0.1/machineresolver1/offline",
            method="DELETE",
            headers={"Authorization": self.at},
        ):
            res = self.app.full_dispatch_request()
