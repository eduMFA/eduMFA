...
class LibPolicyTestCase(MyTestCase):
    ...
    def test_06_passthru(self):
        ...
        set_policy(
            name="pol1", scope=SCOPE.AUTH, action=f"{ACTION.PASSTHRU!s}=userstore"
        )
        g = FakeFlaskG()
        g.policy_object = PolicyClass()
        g.audit_object = FakeAudit()
        ...
        set_policy(
            name="pol1", scope=SCOPE.AUTH, action=f"{ACTION.PASSTHRU!s}=radiusconfig1"
        )
    ...
    def test_10_auth_lastauth(self):
        ...
        set_policy(
            name="pol_lastauth", scope=SCOPE.AUTHZ, action=f"{ACTION.LASTAUTH!s}=1d"
        )
        g = FakeFlaskG()
        g.policy_object = PolicyClass()
        g.audit_object = FakeAudit()
    ...
    def test_13_passthru_priorities(self):
        ...
        radiusmock.setdata(response=radiusmock.AccessAccept)
        set_policy(
            name="pol2", scope=SCOPE.AUTH, action=f"{ACTION.PASSTHRU!s}=radiusconfig1"
        )
        r = add_radius("radiusconfig1", "1.2.3.4", "testing123", dictionary=DICT_FILE)
        self.assertTrue(r > 0)
        g = FakeFlaskG()
    ...
    def test_16_passthru_assign(self):
        ...
        r = check_user_pass(
            User("cornelius", "r1"), f"test{self.valid_otp_values[2]!s}"
        )
        self.assertTrue(r[0])
