...
class TaskSimpleStatsTestCase(MyTestCase):
    ...
    def test_00_read_simplestats_to_monitoringstats(self):
        ...
        for o in sst.options.keys():
            self.assertEqual(
                simple_results[o][0], get_values(o)[0][1], msg=f"Current option: {o}"
            )
        ...
        for o in sst.options.keys():
            self.assertEqual(
                simple_results[o][1], get_values(o)[1][1], msg=f"Current option: {o}"
            )
        ...
        for o in sst.options.keys():
            self.assertEqual(
                simple_results[o][2], get_values(o)[2][1], msg=f"Current option: {o}"
            )
