    ...
    def test_02_get_auth_item(self):
        ...
        self.assertTrue(
            passlib.hash.pbkdf2_sha512.verify(
                "969429", auth_item.get("response").get(3)  # count = 3
            )
        )
        self.assertTrue(
            passlib.hash.pbkdf2_sha512.verify(
                "399871", auth_item.get("response").get(8)  # count = 8
            )
        )
        ...
        self.assertTrue(
            passlib.hash.pbkdf2_sha512.verify(
                "629694", auth_item.get("response").get(102)  # count = 102
            )
        )
...
class BaseApplicationTestCase(MyTestCase):
    ...
    def test_03_allow_bulk_call(self):
        bulk = is_application_allow_bulk_call("edumfa.lib.applications.base")
