...
class TtypePushAPITestCase(MyApiTestCase):
    ...
    def test_01_api_enroll_push(self):
        ...
        with self.app.test_request_context('/ttype/push',
            ...
            self.assertEqual(tokeninfo.get(LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG), self.firebase_config_name)
            # remove the token
            remove_token(serial)
    ...
    def test_03_api_enroll_push_poll_only(self):
        ...
        with self.app.test_request_context('/ttype/push',
            ...
            self.assertEqual(tokeninfo.get("public_key_server").strip().strip("-BEGIN END RSA PUBLIC KEY-").strip(),
                             pubkey)
            # The token should also contain the firebase config
            self.assertEqual(tokeninfo.get(LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG), POLL_ONLY)
...
class TtypeEduPushAPITestCase(MyApiTestCase):
    """
    test /ttype/edupush
    """

    REGISTRATION_URL = "http://test/ttype/edupush"
    ...
    def test_01_api_enroll_push(self):
        ...
        with self.app.test_request_context('/token/init',
            ...
            error = res.json.get("result").get("error")
            self.assertEqual(error.get("message"),
                             "Missing enrollment policy for push token: edupush_firebase_configuration")
            self.assertEqual(error.get("code"), 303)
        ...
        with self.app.test_request_context('/token/init',
            ...
            self.assertTrue("pushurl" in detail)
            # check that the new URL contains the serial number
            self.assertTrue("&serial=EDUP" in detail.get("pushurl").get("value"))
            self.assertFalse("otpkey" in detail)
            enrollment_credential = detail.get("enrollment_credential")
        ...
        with self.app.test_request_context('/ttype/edupush',
            ...
            self.assertEqual(tokeninfo.get("public_key_server").strip().strip("-BEGIN END RSA PUBLIC KEY-").strip(),
                             pubkey)
            # The token should also contain the firebase config
            self.assertEqual(tokeninfo.get(PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG), self.firebase_config_name)
            # remove the token
            remove_token(serial)
    ...
    def test_03_api_enroll_push_poll_only(self):
        ...
        with self.app.test_request_context('/token/init',
            ...
            self.assertIn("pushurl", detail)
            # check that the new URL contains the serial number
            self.assertIn("&serial=EDUP", detail.get("pushurl").get("value"))
            # The firebase settings are NOT contained in the QR Code, since we do poll_only
            # poll_only
            self.assertNotIn("appid=", detail.get("pushurl").get("value"))
            self.assertNotIn("appidios=", detail.get("pushurl").get("value"))
        ...
        with self.app.test_request_context('/ttype/edupush',
            ...
            self.assertEqual(tokeninfo.get("public_key_server").strip().strip("-BEGIN END RSA PUBLIC KEY-").strip(),
                             pubkey)
            # The token should also contain the firebase config
            self.assertEqual(tokeninfo.get(PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG), POLL_ONLY)
