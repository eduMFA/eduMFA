...
class AppsTestCase(MyTestCase):
    ...
    def test_01_apps_urls(self):
        ...
        r = create_google_authenticator_url(tokentype="TOTP", key="123456",
                                            period=60)
        self.assertTrue("otpauth://TOTP/mylabel?secret=CI2FM&period=60&digits=6&issuer=eduMFA" in r, r)
        ...
        self.assertEqual(r, "oathtoken:///addToken?name=mylabel&lockdown=true&key=12345678&timeBased=true")
    ...
    def test_02_extra_data(self):
        ...
        self.assertTrue(r.startswith("oathtoken:///addToken?name=mylabel&lockdown=true&key=12345678&timeBased=true"), r)
        self.assertIn("&somekey=somevalue", r)
        self.assertIn("&s%C3%B6mekey=s%C3%B6mev%C3%A4lue", r)
        self.assertIn("&anotherkey=12345", r)
