...
from edumfa.lib.realm import delete_realm, set_realm as create_realm
...
class PrePolicyDecoratorTestCase(MyApiTestCase):
    ...
    def test_03_check_token_upload(self):
        ...
        set_policy(
            name="pol1",
            scope=SCOPE.ADMIN,
            action=f"enrollTOTP, enrollHOTP, {ACTION.IMPORT!s}",
            client="10.0.0.0/8",
        )
        g.policy_object = PolicyClass()
    ...
    def test_04a_check_max_active_token_user(self):
        ...
        set_policy(
            name="pol1",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.MAXACTIVETOKENUSER!s}={1!s}",
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="pol1",
            scope=SCOPE.ENROLL,
            action=f"hotp_{ACTION.MAXACTIVETOKENUSER!s}={1!s}",
        )
    ...
    def test_04_check_max_token_user(self):
        ...
        set_policy(
            name="pol1", scope=SCOPE.ENROLL, action=f"{ACTION.MAXTOKENUSER!s}={2!s}"
        )
        g.policy_object = PolicyClass()
        # The user has one token, everything is fine.
        self.setUp_user_realms()
        ...
        set_policy(
            name="pol_max_12",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.MAXTOKENUSER!s}={12!s}",
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="pol2",
            scope=SCOPE.ENROLL,
            action=f"hotp_{ACTION.MAXTOKENUSER!s}={2!s}",
        )
        g.policy_object = PolicyClass()
    ...
    def test_06_set_realm(self):
        ...
        set_policy(
            name="pol1",
            scope=SCOPE.AUTHZ,
            action=f"{ACTION.SETREALM!s}={self.realm1!s}",
            realm="somerealm",
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="pol2",
            scope=SCOPE.AUTHZ,
            action=f"{ACTION.SETREALM!s}=ConflictRealm",
            realm="somerealm",
        )
        g.policy_object = PolicyClass()
    ...
    def test_06_set_tokenlabel(self):
        ...
        set_policy(
            name="pol1", scope=SCOPE.ENROLL, action=f"{ACTION.TOKENLABEL!s}=<u>@<r>"
        )
        set_policy(
            name="pol2", scope=SCOPE.ENROLL, action=f"{ACTION.TOKENISSUER!s}=myPI"
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="pol3",
            scope=SCOPE.ENROLL,
            action=f"hotp_{ACTION.FORCE_APP_PIN!s}=True",
        )
        req.all_data = {"user": "cornelius", "realm": "home"}
        init_tokenlabel(req)
    ...
    def test_07a_init_random_pin(self):
        ...
        set_policy(
            name="pinsize",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.OTPPINRANDOM!s}={size_policy!s}",
        )
        set_policy(
            name="pincontent",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.OTPPINCONTENTS!s}={contents_policy!s}",
        )
        set_policy(
            name="pinhandling",
            scope=SCOPE.ENROLL,
    ...
    def test_07b_set_random_pin(self):
        ...
        set_policy(
            name="pinsize",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.OTPPINSETRANDOM!s}={size_policy!s}",
        )
        set_policy(
            name="pincontent",
            scope=SCOPE.ADMIN,
    ...
    def test_12_mangle(self):
        ...
        set_policy(
            name="mangle1",
            scope=SCOPE.AUTH,
            action=f"{ACTION.MANGLE!s}=user/.*(.{{4}}$)/\\1/",
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="mangle2", scope=SCOPE.AUTH, action=f"{ACTION.MANGLE!s}=realm/\\s//"
        )
        g.policy_object = PolicyClass()
    ...
    def test_15_reset_password(self):
        ...
        # Set a mangle policy to change the username
        # and only use the last 4 characters of the username
        set_policy(name="recover", scope=SCOPE.USER, action=f"{ACTION.RESYNC!s}")
        g.policy_object = PolicyClass()
        req.all_data = {"user": "cornelius", "realm": self.realm1}
        ...
        set_policy(name="recover", scope=SCOPE.USER, action=f"{ACTION.PASSWORDRESET!s}")
        g.policy_object = PolicyClass()
    ...
    def test_18_auditlog_age(self):
        ...
        # Set a mangle policy to change the username
        # and only use the last 4 characters of the username
        set_policy(name="a_age", scope=SCOPE.ADMIN, action=f"{ACTION.AUDIT_AGE!s}=1d")
        g.policy_object = PolicyClass()
    ...
    def test_18b_hide_audit_columns(self):
        ...
        set_policy(
            ...
            scope=SCOPE.ADMIN,
            action=f"{ACTION.HIDE_AUDIT_COLUMNS!s}=serial action",
        )
        g.logged_in_user = {"username": "admin1", "realm": "", "role": "admin"}
        # request, that matches the policy
        req.all_data = {"user": "Unknown"}
        ...
        set_policy(
            ...
            scope=SCOPE.USER,
            action=f"{ACTION.HIDE_AUDIT_COLUMNS!s}=number realm",
        )
        g.logged_in_user = {"username": "user1", "realm": "", "role": "user"}
        # request, that matches the policy
        req.all_data = {"user": "Unknown"}
    ...
    def test_18c_hide_tokeninfo(self):
        ...
        set_policy(
            ...
            scope=SCOPE.ADMIN,
            action=f"{ACTION.HIDE_TOKENINFO!s}=tokenkind unknown",
        )
        g.logged_in_user = {"username": "admin1", "realm": "", "role": "admin"}
        req.all_data = {}
        req.User = User("Unknown")
    ...
    def test_19_tantoken_count(self):
        ...
        req = Request(env)
        set_policy(
            name="tanpol", scope=SCOPE.ENROLL, action=f"{TANACTION.TANTOKEN_COUNT!s}=10"
        )
        g.policy_object = PolicyClass()
    ...
    def test_22a_push_firebase_config(self):
        ...
        set_policy(
            name="push_pol2",
            scope=SCOPE.ENROLL,
            action=f"{PushTokenClass.PUSH_ACTION.SSL_VERIFY!s}=0",
        )
        g.policy_object = PolicyClass()
        req.all_data = {"type": "edupush"}
        pushtoken_add_config(req, "init")
    ...
    def test_22b_push_firebase_config(self):
        ...
        set_policy(
            name="push_pol2",
            scope=SCOPE.ENROLL,
            action=f"{LegacyPushTokenClass.PUSH_ACTION.SSL_VERIFY!s}=0",
        )
        g.policy_object = PolicyClass()
        req.all_data = {"type": "push"}
        pushtoken_add_config(req, "init")
    ...
    def test_24_push_wait_policy(self):
        ...
        set_policy(
            name="push1",
            scope=SCOPE.AUTH,
            action=f"{LegacyPushTokenClass.PUSH_ACTION.WAIT!s}=10",
        )
        req.all_data = {}
        g.policy_object = PolicyClass()
        legacypushtoken_wait(req, None)
    ...
    def test_36_init_registrationcode_length_contents(self):
        ...
        set_policy(
            name="reg_length",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.REGISTRATIONCODE_LENGTH!s}={6!s}",
        )
        set_policy(
            name="reg_contents",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.REGISTRATIONCODE_CONTENTS!s}=+n",
        )
        # request, that matches the policy
        req.all_data = {"user": "cornelius", "realm": "home", "type": "registration"}
        init_token_length_contents(req)
    ...
    def test_37_init_password_length_contents(self):
        ...
        set_policy(
            name="pw_length",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.PASSWORD_LENGTH!s}={6!s}",
        )
        set_policy(
            name="pw_contents",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.PASSWORD_CONTENTS!s}=+n",
        )
        # request, that matches the policy
    ...
    def test_40_custom_user_attributes(self):
        ...
        set_policy(
            "set_custom_attr",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.DELETE_USER_ATTRIBUTES!s}=department sth",
        )
        req.all_data = {
            "user": "cornelius",
            "realm": self.realm1,
        ...
        set_policy(
            "set_custom_attr2",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.DELETE_USER_ATTRIBUTES!s}=difkey",
        )
        req.all_data = {"user": "cornelius", "realm": self.realm1, "attrkey": "difkey"}
        check_custom_user_attributes(req, "delete")
        ...
        set_policy(
            "set_custom_attr2",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.DELETE_USER_ATTRIBUTES!s}=*",
        )
        req.all_data = {
            "user": "cornelius",
            "realm": self.realm1,
        ...
        set_policy(
            "set_custom_attr",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.SET_USER_ATTRIBUTES!s}=:department: finance devel :color: * :*: 1 2 ",
        )
        ...
        set_policy(
            "set_custom_attr2",
            scope=SCOPE.ADMIN,
            action=f"{ACTION.SET_USER_ATTRIBUTES!s}=:*: *",
        )
        req.all_data = {
            "user": "cornelius",
            "realm": self.realm1,
    ...
    def test_61_required_description_for_specified_token_types(self):
        ...
        set_policy(
            ...
            scope=SCOPE.ENROLL,
            action=[f"{ACTION.REQUIRE_DESCRIPTION!s}=hotp"],
        )
        req = Request(env)
...
class PostPolicyDecoratorTestCase(MyApiTestCase):
    ...
    def test_05_autoassign_any_pin(self):
        ...
        set_policy(
            name="pol2",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.AUTOASSIGN!s}={AUTOASSIGNVALUE.NONE!s}",
            client="10.0.0.0/8",
        )
        g.policy_object = PolicyClass()
    ...
    def test_05_autoassign_userstore(self):
        ...
        set_policy(
            name="pol2",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.AUTOASSIGN!s}={AUTOASSIGNVALUE.USERSTORE!s}",
            client="10.0.0.0/8",
        )
        g.policy_object = PolicyClass()
    ...
    def test_08_get_webui_settings(self):
        ...
        set_policy(
            name="pol_qr3",
            scope=SCOPE.WEBUI,
            action=f"{ACTION.SHOW_CUSTOM_AUTHENTICATOR!s}=https://edumfa.io",
        )
        ...
        set_policy(
            ...
            scope=SCOPE.WEBUI,
            action=f"{ACTION.LOGOUT_REDIRECT!s}={redir_uri!s}",
        )
        g.policy_object = PolicyClass()
        new_response = get_webui_settings(req, resp)
        jresult = new_response.json
    ...
    def test_09_get_webui_settings_token_pagesize(self):
        ...
        set_policy(
            ...
            scope=SCOPE.WEBUI,
            realm=self.realm1,
            action=f"{ACTION.TOKENPAGESIZE!s}=177",
        )
        g.policy_object = PolicyClass()
        new_response = get_webui_settings(req, resp)
        jresult = new_response.json
    ...
    def test_17_pin_change(self):
        ...
        set_policy(
            name="pol2", scope=SCOPE.ENROLL, action=f"{ACTION.CHANGE_PIN_EVERY!s}=1d"
        )
        g.policy_object = PolicyClass()
        save_pin_change(req, resp)
        ti = token.get_tokeninfo("next_pin_change")
    ...
    def test_18_challenge_text_header(self):
        ...
        set_policy(
            name="pol_header",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_HEADER!s}=These are your options:<ul>",
        )
        # Set a policy for the footer
        set_policy(
            name="pol_footer",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_FOOTER!s}=Happy authenticating!",
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            name="pol_header",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_HEADER!s}=These are your options:",
        )
        g.policy_object = PolicyClass()
        resp = jsonify(res)
    ...
    def test_19_is_authorized(self):
        ...
        set_policy(
            "auth01",
            scope=SCOPE.AUTHZ,
            action=f"{ACTION.AUTHORIZED!s}={AUTHORIZED.DENY!s}",
            priority=2,
        )
        g.policy_object = PolicyClass()
        ...
        set_policy(
            "auth02",
            scope=SCOPE.AUTHZ,
            action=f"{ACTION.AUTHORIZED!s}={AUTHORIZED.ALLOW!s}",
            priority=1,
            client="10.0.0.0/8",
        )
        g.policy_object = PolicyClass()
    ...
    def test_20_verify_enrollment(self):
        ...
        set_policy(
            "verify_toks",
            scope=SCOPE.ENROLL,
            action=f"{ACTION.VERIFY_ENROLLMENT!s}=hotp",
        )
        g.policy_object = PolicyClass()
