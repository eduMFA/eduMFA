...
class PushTokenTestCase(MyTestCase):
    ...
    def _create_push_token(self):
        ...
        tok.add_tokeninfo(LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG, self.firebase_config_name)
    ...
    def test_01a_enroll_with_app_pin(self):
        ...
        tparams.update(FB_CONFIG_VALS)
        tok = init_token(param=tparams)
        detail = tok.get_init_detail(params={LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG: POLL_ONLY,
    ...
    def test_02a_lib_enroll(self):
        ...
        set_policy("push1", scope=SCOPE.ENROLL,
                   action="{0!s}={1!s}".format(LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG,
    ...
    def test_03a_api_authenticate_fail(self):
        ...
        set_policy("push1", scope=SCOPE.ENROLL,
                   action="{0!s}={1!s},{2!s}={3!s},{4!s}={5!s}".format(
                       ...
                       self.firebase_config_name,
                       LegacyPushTokenClass.PUSH_ACTION.REGISTRATION_URL, REGISTRATION_URL,
                       LegacyPushTokenClass.PUSH_ACTION.TTL, TTL))
        # create push token
        ...
        with mock.patch('edumfa.lib.smsprovider.FirebaseProvider.service_account.Credentials'
            ...
            set_policy('push_poll', SCOPE.AUTH,
                       action='{0!s}={1!s}'.format(LegacyPushTokenClass.PUSH_ACTION.ALLOW_POLLING,
    ...
    def test_15_poll_endpoint(self):
        ...
        with mock.patch('edumfa.models.datetime') as mock_dt1,\
            ...
            mock_dt1.now.return_value = timestamp.replace(tzinfo=None) + timedelta(seconds=15)
        ...
        with mock.patch('edumfa.models.datetime') as mock_datetime:
            mock_datetime.now.return_value = challenge_timestamp.replace(tzinfo=None)
        ...
        tok.add_tokeninfo(LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG, 'my unknown firebase config')
        req.all_data = {'serial': serial,
                        'timestamp': ts,
                        'signature': b32encode(sig)}
        # poll for challenges
...
class EduPushTokenTestCase(MyTestCase):
    ...
    def _create_push_token(self):
        tparams = {'type': 'edupush', 'genkey': 1}
        tparams.update(FB_CONFIG_VALS)
        tok = init_token(param=tparams)
        tok.add_tokeninfo(PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG, self.firebase_config_name)
        tok.add_tokeninfo(PUBLIC_KEY_SMARTPHONE, self.smartphone_public_key_pem_urlsafe)
        tok.add_tokeninfo('firebase_token', 'firebaseT')
        tok.add_tokeninfo(PUBLIC_KEY_SERVER, self.server_public_key_pem)
        tok.add_tokeninfo(PRIVATE_KEY_SERVER, self.server_private_key_pem, 'password')
    ...
    def test_01_create_token(self):
        db_token = Token(self.serial1, tokentype="edupush")
        db_token.save()
        token = PushTokenClass(db_token)
        self.assertEqual(token.token.serial, self.serial1)
        self.assertEqual(token.token.tokentype, "edupush")
        self.assertEqual(token.type, "edupush")
        class_prefix = token.get_class_prefix()
        self.assertEqual(class_prefix, "EDUP")
        self.assertEqual(token.get_class_type(), "edupush")
        ...
        fb_config = {FIREBASE_CONFIG.REGISTRATION_URL: "http://test/ttype/edupush",
                     FIREBASE_CONFIG.JSON_CONFIG: CLIENT_FILE,
                     FIREBASE_CONFIG.TTL: 10}
    ...
    def test_01a_enroll_with_app_pin(self):
        ...
        tparams.update(FB_CONFIG_VALS)
        tok = init_token(param=tparams)
        detail = tok.get_init_detail(params={PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG: POLL_ONLY,
    ...
    def test_02a_lib_enroll(self):
        ...
        self.assertTrue(r > 0)
        set_policy("push1", scope=SCOPE.ENROLL,
                   action="{0!s}={1!s}".format(PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG,
                                               self.firebase_config_name))
        token_obj = self._create_push_token()
        remove_token(token_obj.get_serial())
    ...
    def test_03a_api_authenticate_fail(self):
        ...
        with mock.patch('edumfa.lib.smsprovider.FirebaseProvider.service_account.Credentials'
            ...
            # disallow polling through a policy
            set_policy('push_poll', SCOPE.AUTH,
                       action='{0!s}={1!s}'.format(PushTokenClass.PUSH_ACTION.ALLOW_POLLING,
                                                   PushAllowPolling.DENY))
    ...
    @responses.activate
    def test_03b_api_authenticate_client(self):
        # Test the /validate/check endpoints without the smartphone endpoint /ttype/edupush
        self.setUp_user_realms()
        # create FireBase Service and policies
        set_smsgateway(self.firebase_config_name,
                       'edumfa.lib.smsprovider.FirebaseProvider.FirebaseProvider',
    ...
    @responses.activate
    def test_04_api_authenticate_smartphone(self):
        # Test the /validate/check endpoints and the smartphone endpoint /ttype/edupush
        # for authentication

        # get enrolled push token
        toks = get_tokens(tokentype="edupush")
        self.assertEqual(len(toks), 1)
        tokenobj = toks[0]
    ...
    def test_15_poll_endpoint(self):
        ...
        # unknown firebase configuration
        tok.add_tokeninfo(PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG, 'my unknown firebase config')
        req.all_data = {'serial': serial,
                        'timestamp': ts,
                        'signature': b32encode(sig)}
        # poll for challenges
