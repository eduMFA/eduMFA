...
class PushTokenTestCase(MyTestCase):
    ...
    def test_03a_api_authenticate_fail(self):
        ...
        set_policy(
            ...
            scope=SCOPE.ENROLL,
            action=f"{LegacyPushTokenClass.PUSH_ACTION.FIREBASE_CONFIG}={self.firebase_config_name},"
            f"{LegacyPushTokenClass.PUSH_ACTION.REGISTRATION_URL}={REGISTRATION_URL},"
    ...
    def test_03b_api_authenticate_client(self):
        ...
        with mock.patch("edumfa.lib.smsprovider.FirebaseProvider.time") as mock_time:
            ...
            with mock.patch(
                ...
                set_policy(
                    "push1",
                    scope=SCOPE.AUTH,
                    action=f"{LegacyPushTokenClass.PUSH_ACTION.WAIT!s}=20",
                )
                # Send the first authentication request to trigger the challenge
                with self.app.test_request_context(
                    "/validate/check",
        ...
        with mock.patch(
            ...
            set_policy(
                "push1",
                scope=SCOPE.AUTH,
                action=f"{LegacyPushTokenClass.PUSH_ACTION.WAIT!s}=1",
            )
            # Send the first authentication request to trigger the challenge
            with self.app.test_request_context(
                "/validate/check",
    ...
    def test_06_api_auth(self):
        ...
        set_policy(
            "webui", scope=SCOPE.WEBUI, action=f"{ACTION.LOGINMODE}={LOGINMODE.EDUMFA}"
        )
    ...
    def test_07_check_timestamp(self):
        ...
        self.assertRaisesRegex(
            eduMFAError,
            rf"Could not parse timestamp {timestamp_fmt}. ISO-Format required.",
        ...
        with mock.patch("edumfa.lib.tokens.pushtoken.datetime") as mock_dt:
            mock_dt.now.return_value = timestamp - timedelta(minutes=9)
            LegacyPushTokenClass._check_timestamp_in_range(timestamp.isoformat(), 10)
        timestamp_iso = timestamp.isoformat().replace("+", r"\+")
        with mock.patch("edumfa.lib.tokens.pushtoken.datetime") as mock_dt:
            mock_dt.now.return_value = timestamp + timedelta(minutes=9)
            self.assertRaisesRegex(
                eduMFAError,
                rf"Timestamp {timestamp_iso} not in valid range.",
                LegacyPushTokenClass._check_timestamp_in_range,
                timestamp.isoformat(),
                8,
            )
...
class EduPushTokenTestCase(MyTestCase):
    ...
    def test_01_create_token(self):
        ...
        fb_config = {
            FIREBASE_CONFIG.REGISTRATION_URL: "http://test/ttype/edupush",
            FIREBASE_CONFIG.JSON_CONFIG: CLIENT_FILE,
            FIREBASE_CONFIG.TTL: 10,
        }
    ...
    def test_03b_api_authenticate_client(self):
        ...
        with mock.patch("edumfa.lib.smsprovider.FirebaseProvider.time") as mock_time:
            ...
            with mock.patch(
                ...
                set_policy(
                    "push1",
                    scope=SCOPE.AUTH,
                    action=f"{PushTokenClass.PUSH_ACTION.WAIT!s}=20",
                )
                # Send the first authentication request to trigger the challenge
                with self.app.test_request_context(
                    "/validate/check",
        ...
        with mock.patch(
            ...
            set_policy(
                "push1",
                scope=SCOPE.AUTH,
                action=f"{PushTokenClass.PUSH_ACTION.WAIT!s}=1",
            )
            # Send the first authentication request to trigger the challenge
            with self.app.test_request_context(
                "/validate/check",
    ...
    def test_06_api_auth(self):
        ...
        # Set a PUSH_WAIT action which will be ignored by eduMFA
        set_policy(
            "push1", scope=SCOPE.AUTH, action=f"{PushTokenClass.PUSH_ACTION.WAIT!s}=20"
        )
        with mock.patch(
            "edumfa.lib.smsprovider.FirebaseProvider.service_account.Credentials"
            ".from_service_account_file"
    ...
    def test_15_poll_endpoint(self):
        ...
        # unknown firebase configuration
        tok.add_tokeninfo(
            PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG, "my unknown firebase config"
        )
        req.all_data = {"serial": serial, "timestamp": ts, "signature": b32encode(sig)}
        # poll for challenges
        with mock.patch("edumfa.models.datetime") as mock_dt1, mock.patch(
