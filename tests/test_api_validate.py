...
from edumfa.models import AuthCache, Challenge, Policy, Realm, Token, TokenOwner, db
...
class AuthorizationPolicyTestCase(MyApiTestCase):
    ...
    def test_02_enroll_tokens(self):
        ...
        self.assertTrue(r)
        self.assertEqual(f"{r.user}", "<frank.catchall@ldaprealm>")
...
class ValidateAPITestCase(MyApiTestCase):
    ...
    def test_23_pass_no_user_and_pass_no_token(self):
        ...
        set_policy(
            name="pass_no",
            scope=SCOPE.AUTH,
            action=f"{ACTION.PASSNOTOKEN},{ACTION.PASSNOUSER}",
        )
        ...
        with self.app.test_request_context(
            ...
            detail = res.json.get("detail")
            self.assertEqual(
                detail.get("message"), "user does not exist, accepted due to 'pass_no'"
            )
    ...
    def test_25_trigger_challenge_smtp(self):
        ...
        set_policy(
            "emailtext",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILTEXT}=Dein <otp>",
        )
        set_policy(
            "emailsubject",
            scope=SCOPE.AUTH,
            action=f"{EMAILACTION.EMAILSUBJECT}=Dein OTP",
        )
    ...
    def test_30_return_different_tokentypes(self):
        ...
        set_policy("onlyHOTP", scope=SCOPE.AUTHZ, action=f"{ACTION.TOKENTYPE}=hotp")
        ...
        set_policy(
            "passthru",
            scope=SCOPE.AUTH,
            action=f"{ACTION.PASSTHRU}=userstore",
        )
    ...
    def test_30_challenge_text(self):
        ...
        set_policy(
            "chalsms",
            SCOPE.AUTH,
            f"sms_{ACTION.CHALLENGETEXT}=check your sms",
        )
        set_policy(
            "chalemail",
            SCOPE.AUTH,
            f"email_{ACTION.CHALLENGETEXT}=check your email",
        )
    ...
    def test_35_application_tokentype(self):
        ...
        set_policy(
            name="pol_chalresp",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGERESPONSE}=hot totp",
        )
    ...
    def test_36_authorize_by_tokeninfo_condition(self):
        ...
        set_policy(
            ...
            action=f"{ACTION.AUTHORIZED}=deny_access",
            scope=SCOPE.AUTHZ,
            priority=100,
        )
        ...
        set_policy(
            ...
            action=f"{ACTION.AUTHORIZED}=grant_access",
            scope=SCOPE.AUTHZ,
            priority=1,
    ...
    def test_39_invalid_user(self):
        ...
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "eve", "pass": "1234"}
        ):
            res = self.app.full_dispatch_request()
            ...
            self.assertEqual(
                f"ERR904: User <eve@{self.realm3}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <eve@{self.realm1}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <eve.{self.resolvername1}@{self.realm3}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <eve.{self.resolvername1}@{self.realm1}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": user.login, "pass": "1234"}
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(200, res.status_code, res)
            result = res.json["detail"]
            self.assertEqual(
                "The user has no tokens assigned", result["message"], result
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <{user_realm1.login}@{self.realm3}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <{user.login}@{user.realm}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <{user.login}.{user.resolver}@{user.realm}> does not exist.",
                error.get("message"),
                error,
            )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(904, error.get("code"), error)
            self.assertEqual(
                f"ERR904: User <{user.login}.{self.resolvername3}@{user.realm}> does not exist.",
                error.get("message"),
                error,
            )
...
class RegistrationAndPasswordToken(MyApiTestCase):
    ...
    def test_01_password_tokens(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": quote(f"test{password}")},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            data = res.json
...
class MultiChallege(MyApiTestCase):
    ...
    def test_01_pin_change_via_validate_single_shot(self):
        ...
        with self.app.test_request_context(
            ...
            data={
                "user": "cornelius",
                "pass": f"test{self.valid_otp_values[1]}",
            },
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(res.status_code, 200)
    ...
    def test_02_challenge_text_header(self):
        ...
        set_policy(
            "challenge_header",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_HEADER}={challenge_header}",
        )
    ...
    def test_03_preferred_client_mode(self):
        ...
        set_policy(
            "push2",
            scope=SCOPE.ENROLL,
            action=f"{PushTokenClass.PUSH_ACTION.FIREBASE_CONFIG}={POLL_ONLY},{PushTokenClass.PUSH_ACTION.REGISTRATION_URL}={REGISTRATION_URL},{PushTokenClass.PUSH_ACTION.TTL}={TTL}",
        )
...
class AChallengeResponse(MyApiTestCase):
    ...
    def test_08_challenge_text(self):
        ...
        set_policy(
            name="pol_footer",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_FOOTER}=</ul>.<b>Authenticate Now!</b>",
        )
...
class MultiChallengeEnrollTest(MyApiTestCase):
    ...
    def test_02_enroll_TOTP(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE}=totp",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
    ...
    def test_03_enroll_EMail(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE}=email",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
    ...
    def test_04_enroll_SMS(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE}=sms",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
...
class ValidateShortPasswordTestCase(MyApiTestCase):
    ...
    def test_00_setup_tokens(self):
        ...
        with self.app.test_request_context(
            ...
            data={
                "user": "cornelius",
                "pass": f"{pin}{self.valid_otp_values[0]}",
            },
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
        ...
        with self.app.test_request_context(
            ...
            data={
                "user": "cornelius",
                "pass": f"{pin}{self.valid_yubi_otps[0]}",
            },
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
