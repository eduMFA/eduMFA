...
class ValidateAPITestCase(MyApiTestCase):
    ...
    def test_17_auth_timelimit_success(self):
        ...
        set_policy(
            name="pol_time1",
            scope=SCOPE.AUTHZ,
            action=f"{ACTION.AUTHMAXSUCCESS!s}=2/20s",
        )
    ...
    def test_18_auth_timelimit_fail(self):
        ...
        # set policy for timelimit
        set_policy(
            name="pol_time1", scope=SCOPE.AUTHZ, action=f"{ACTION.AUTHMAXFAIL!s}=2/20s"
        )
    ...
    def test_21_validate_disabled(self):
        ...
        set_policy(
            name="disabled", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=userstore"
        )
        # enroll two tokens
        r = init_token(
    ...
    def test_22_validate_locked(self):
        ...
        user = "lockeduser"
        set_policy(
            name="locked", scope=SCOPE.AUTH, action=f"{ACTION.OTPPIN!s}=tokenpin"
        )
        r = init_token(
    ...
    def test_23_pass_no_user_and_pass_no_token(self):
        ...
        set_policy(
            name="pass_no",
            scope=SCOPE.AUTH,
            action=f"{ACTION.PASSNOTOKEN!s},{ACTION.PASSNOUSER!s}",
        )
    ...
    def test_30_return_different_tokentypes(self):
        ...
        set_policy("onlyHOTP", scope=SCOPE.AUTHZ, action=f"{ACTION.TOKENTYPE!s}=hotp")
        ...
        set_policy(
            "passthru", scope=SCOPE.AUTH, action=f"{ACTION.PASSTHRU!s}=userstore"
        )
    ...
    def test_30_challenge_text(self):
        ...
        set_policy(
            "chalsms", SCOPE.AUTH, f"sms_{ACTION.CHALLENGETEXT!s}=check your sms"
        )
        set_policy(
            "chalemail", SCOPE.AUTH, f"email_{ACTION.CHALLENGETEXT!s}=check your email"
        )
    ...
    def test_33_auth_cache(self):
        ...
        set_policy(
            name="authcache", action=f"{ACTION.AUTH_CACHE!s}=4m", scope=SCOPE.AUTH
        )
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
    ...
    def test_35_application_tokentype(self):
        ...
        set_policy(
            name="pol_chalresp",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGERESPONSE!s}=hot totp",
        )
    ...
    def test_36_authorize_by_tokeninfo_condition(self):
        ...
        set_policy(
            ...
            action=f"{ACTION.AUTHORIZED!s}=deny_access",
            scope=SCOPE.AUTHZ,
            priority=100,
        )
        ...
        set_policy(
            ...
            action=f"{ACTION.AUTHORIZED!s}=grant_access",
            scope=SCOPE.AUTHZ,
            priority=1,
...
class RegistrationAndPasswordToken(MyApiTestCase):
    ...
    def test_01_password_tokens(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": quote(f"test{password!s}")},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            data = res.json
...
↵uj0TB5-XjlmAvivyWLi9WSI7pCJYIEJicw0LtP_hdy8yh6ANEUXBJsWtkGDci9DcN1rDG1tE"""
    ...
    def test_12_usernameless_authentication(self):
        ...
        with self.app.test_request_context(
            ...
            webauthn_sign_request = (
                data.get("detail").get("attributes").get("webAuthnSignRequest")
            )
    ...
    def test_12b_usernameless_realm_policy(self):
        ...
        set_policy(
            ...
            scope=SCOPE.AUTH,
            action=f"{WEBAUTHNACTION.TIMEOUT!s}=42",
            realm="foo-realm",
        )
    ...
    def test_12c_ensure_conditions_userless(self):
        ...
        set_policy(
            name="pol_chalresp",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGERESPONSE!s}=hotp totp",
        )
        with self.app.test_request_context(
            "/validate/triggerchallenge",
            method="POST",
    ...
    def test_21_validate_webauthn_triggerchallenge(self):
        ...
        with self.app.test_request_context(
            ...
            for token in data.get("detail").get("multi_challenge"):
                for credential in (
                    token.get("attributes")
                    .get("webAuthnSignRequest")
                    .get("allowCredentials")
                ):
                    self.assertTrue(
                        (
                            set(DEFAULT_ALLOWED_TRANSPORTS.split())
                            ^ set(credential.get("transports"))
                        )
                        == set()
                    )
            self.assertEqual(self.serial, data.get("detail").get("serial"))
...
class MultiChallege(MyApiTestCase):
    ...
    def test_00_pin_change_via_validate_chalresp(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": f"newpin{self.valid_otp_values[2]!s}"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(res.status_code, 200)
            result = res.json["result"]
    ...
    def test_01_pin_change_via_validate_single_shot(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": f"test{self.valid_otp_values[1]!s}"},
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(res.status_code, 200)
            result = res.json["result"]
    ...
    def test_02_challenge_text_header(self):
        ...
        set_policy(
            "challenge_header",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_HEADER!s}={challenge_header!s}",
        )
        ...
        with self.app.test_request_context(
            ...
            self.assertEqual(
                f"{challenge_header!s}<li>Please enter a new PIN</li>\n",
                details.get("message"),
            )
...
class AChallengeResponse(MyApiTestCase):
    ...
    def test_01_challenge_response_token_deactivate(self):
        ...
        set_policy(
            name="pol_cr", scope=SCOPE.AUTH, action=f"{ACTION.CHALLENGERESPONSE!s}=hotp"
        )
    ...
    def test_08_challenge_text(self):
        ...
        set_policy(
            name="pol_footer",
            scope=SCOPE.AUTH,
            action=f"{ACTION.CHALLENGETEXT_FOOTER!s}=</ul>.<b>Authenticate Now!</b>",
        )
    ...
    def test_10_unique_transaction_id(self):
        ...
        set_policy(
            "chalresp", scope=SCOPE.AUTHZ, action=f"{ACTION.TRIGGERCHALLENGE!s}=hotp"
        )
    ...
    def test_12_polltransaction(self):
        ...
        entry = self.find_most_recent_audit_entry(action="*/validate/polltransaction*")
        self.assertEqual(entry["action_detail"], f"transaction_id: {transaction_id}")
        self.assertEqual(entry["info"], "status: pending")
        self.assertIn("tok1", entry["serial"])
        self.assertIn("tok2", entry["serial"])
        ...
        entry = self.find_most_recent_audit_entry(action="*/validate/polltransaction*")
        self.assertEqual(
            entry["action_detail"], f"transaction_id: {old_transaction_id}"
        )
        self.assertEqual(entry["info"], "status: pending")
        self.assertEqual(entry["serial"], None)
        self.assertFalse(entry["success"])
        ...
        with self.app.test_request_context(
            f"/validate/polltransaction/{transaction_id}", method="GET", data={}
        ):
            res = self.app.full_dispatch_request()
            self.assertEqual(res.status_code, 200)
            self.assertTrue(res.json["result"]["status"])
        ...
        with self.app.test_request_context(
            f"/validate/polltransaction/{transaction_id}",
            method="GET",
            data={"transaction_id": transaction_id},
        ):
            res = self.app.full_dispatch_request()
...
class TriggeredPoliciesTestCase(MyApiTestCase):
    ...
    def test_00_two_policies(self):
        ...
        set_policy("lastauth", scope=SCOPE.AUTHZ, action=f"{ACTION.LASTAUTH!s}=1s")
...
class MultiChallengeEnrollTest(MyApiTestCase):
    ...
    def test_01_enroll_HOTP(self, capture):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE!s}=hotp",
        )
    ...
    def test_02_enroll_TOTP(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE!s}=totp",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
    ...
    def test_03_enroll_EMail(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE!s}=email",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
    ...
    def test_04_enroll_SMS(self):
        ...
        set_policy(
            "pol_multienroll",
            scope=SCOPE.AUTH,
            action=f"{ACTION.ENROLL_VIA_MULTICHALLENGE!s}=sms",
        )
        # Now we should get an authentication Challenge
        with self.app.test_request_context(
            "/validate/check", method="POST", data={"user": "alice", "pass": "alicepw"}
...
class ValidateShortPasswordTestCase(MyApiTestCase):
    ...
    def test_00_setup_tokens(self):
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": f"{pin!s}{self.valid_otp_values[0]!s}"},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            result = res.json.get("result")
        ...
        with self.app.test_request_context(
            "/validate/check",
            method="POST",
            data={"user": "cornelius", "pass": f"{pin!s}{self.valid_yubi_otps[0]!s}"},
        ):
            res = self.app.full_dispatch_request()
            self.assertTrue(res.status_code == 200, res)
            result = res.json.get("result")
