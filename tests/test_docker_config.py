...
class DockerEnvConfigTestCase(unittest.TestCase):
    """Test cases for the configuration parsing functions in `edumfa_config.py`.

    Note that this is missing a few options which are only parsed in the
    entrypoint (at the time of writing the three admin options: enable, username
    and password). As the entrypoint will likely get rewritten in Python to
    prepare for a distroless image, testing it will wait until then.
    """
    ...
    def test_dict_conversion_malformed(self):
        ...
        with self.assertRaises(ValueError):
            config.str_to_dict("{'invalid': syntax}", "TEST_OPTION")
        config = self._import_config_with_env({})
    ...
    def test_dict_conversion_non_dict(self):
        ...
        config = self._import_config_with_env({})
        with self.assertRaises(ValueError):
            config.str_to_dict("['list', 'of', 'values']", "TEST_OPTION")

    def test_dict_conversion_valid(self):
        """Test str_to_dict with valid dict input"""
        config = self._import_config_with_env({})
        result = config.str_to_dict("{'key': 'value', 'num': 42}", "TEST_OPTION")
        self.assertEqual(result, {"key": "value", "num": 42})

    def test_dict_conversion_empty(self):
        """Test str_to_dict with empty dict input"""
        config = self._import_config_with_env({})
        result = config.str_to_dict("{}", "TEST_OPTION")
        self.assertEqual(result, {})
        # Make sure an empty string causes the option to be unset, as
        # literal_eval throws a SyntaxError when ecountered with "".
        config = self._import_config_with_env(
            {"SQLALCHEMY_ENGINE_OPTIONS": "", "EDUMFA_AUDIT_SQL_OPTIONS": ""}
        )
