...
class SmtpSMSTestCase(MyTestCase):
    ...
    def test_08_smsgateway_success(self):
        ...
        with mock.patch("logging.Logger.debug") as log:
            ...
            self.assertTrue(r)
            log.assert_any_call(
                "submitting message {!r} to {!s}".format("Halo", "123456")
            )
    ...
    def test_09_send_sms_regexp_success(self):
        ...
        with mock.patch("logging.Logger.debug") as log:
            ...
            self.assertTrue(r)
            log.assert_any_call(
                "submitting message {!r} to {!s}".format("Hello", "4912345678")
            )
...
class SipgateSMSTestCase(MyTestCase):
    ...
    def test_08_smsgateway_success(self):
        ...
        with mock.patch("logging.Logger.debug") as log:
            ...
            self.assertTrue(r)
            log.assert_any_call(
                "submitting message {!r} to {!s}".format("Hello", "123456")
            )


class ScriptSMSTestCase(MyTestCase):
    directory = f"{os.getcwd()}/tests/testdata/scripts/"
...
class SmppSMSTestCase(MyTestCase):
    ...
    def test_01_success(self):
        # Here we need to send the SMS
        smppmock.setdata(connection_success=True, systemid="eduMFA", password="secret")
        r = self.provider.submit_message("123456", "Hello")
        self.assertTrue(r)

    @smppmock.activate
    def test_02_fail_connection(self):
        smppmock.setdata(connection_success=False, systemid="eduMFA", password="secret")
        self.assertRaises(SMSError, self.provider.submit_message, "123456", "hello")

    @smppmock.activate
    def test_03_fail_wrong_credentials(self):
        smppmock.setdata(connection_success=True, systemid="eduMFA", password="wrong")
        self.assertRaises(SMSError, self.provider.submit_message, "123456", "hello")
...
class HttpMessageToUidTestCase(MyTestCase):
    ...
    @responses.activate
    def test_01_success(self):
        responses.add(responses.POST, self.url, body=self.success_body)
        # Here we need to send the SMS
    ...
    @responses.activate
    def test_02_fail(self):
        responses.add(responses.POST, self.url, body=self.fail_body)
        # Here we need to send the SMS
        self.assertRaises(SMSError, self.provider.submit_message, "testuser", "Hello")
    ...
    def test_06_send_sms_get_fail(self):
        ...
        # Here we need to send the SMS
        self.assertRaises(
            SMSError, self.get_provider.submit_message, "testuser", "Hello"
        )
    ...
    def test_07_missing_fail(self):
        responses.add(responses.GET, self.missing_url)
        self.assertRaises(
            SMSError, self.missing_provider.submit_message, "testuser", "Hello"
        )
    ...
    def test_11_send_sms_post_success_as_json(self):
        ...
        responses.add(
            ...
            "http://some.other.service",
            body=self.success_body,
            match=[json_params_matcher({"text": "Hello: 7"})],
        )
        # Here we need to send the SMS
        with mock.patch("logging.Logger.debug") as mock_log:
